# Work-Center Capacity Board + Employee×Day Crew Matrix: Best Practices Survey

> Companion prototype: `.ai/research/crew-capacity-prototype.xlsx` (working Excel model of both views)
> Prior art: `.ai/research/daily-crew-station-assignment.md` (the crew board v1 research)
...
manning boards. Carbon's crew board occupying both sides (capacity summary + named daily
...
- **Epicor**: no person×day surface at all. Crew size multiplies costed labor hours only;
...
- **APICS**: the closest formal artifact is the **dispatch list** (work-center-oriented, order
  ...
  supervisor.
- **Rationale**: this is the gap Carbon's crew board fills — most shops bridge it with whiteboards.
...
- **Rationale**: the decisive advantage of doing this inside Carbon is that demand, scheduled, and
  ...
  calendars, crew assignments) — only the assignment layer is manual.
...
6. **Skills × daily crew** — ILUO matrix consulted at the huddle; in software, qualification
   filters the picker and warns on violation (Carbon's crew board amber-warning already matches).

## Recommended Approach for Carbon (crew page)

1. **Add a "Capacity" view to the Schedule area** (sibling of Crew/Kanban/Timeline): rows = work
   ...
   **Scheduled** (capacityReservation hours placed in the bucket — Carbon's finite engine gives us
   the series SAP/Epicor can't show without APS), **Available** (crew headcount × shift hours for
   crewed dates; work-center calendar hours otherwise; minus absences). Load% = Scheduled/Available
...
2. **Make the crew page a true matrix** with both orientations: employee-rows × day-columns
...
3. **Coverage strip** on the matrix: per work-center/day, assigned headcount vs required (required
   from demand hours ÷ shift hours, or a manually-set min-crew per work center) with variance
...
- Epicor: epiusers.help/t/shop-load-report-wrkctrcap-calculations/91741 · /t/overload-informer/107525 · /t/scheduling-multiple-resources-when-to-use-crew-size-vs-machines/116744 · /t/what-are-dailycapacity1-thru-7-on-the-resourcegroup/132396
...
3. **Labor-model spectrum** (strongest→weakest): PlanetTogether (dual finite seizure machine +
   ...
   Attention %, crew size speeds ops) > Schedlyzer (dual-resource, "skills rather than specific
...
4. **Person×day matrix: NO surveyed tool has a true attendance-driven one.** Closest: E2's
   ...
   LillyWorks, Fulcrum, Katana. **Carbon's crew board + matrix would exceed every incumbent.**
