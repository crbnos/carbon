# Daily People / Station Assignment Research: Best Practices Survey
...
**(3)** no surveyed product closes the loop from the day's people into a finite
...
- **Epicor Kinetic (+ Advanced MES/Mattec)** — job-shop reference; approximates
  people via operators-as-resources and people-size multipliers.
...
### 1. The daily people plan is an execution-layer artifact, distinct from both the HR roster and labor actuals
...
- **PlanetTogether**: explicit tri-modal — Named Employees / Shared
  ...
  people-size dial; labor data importable from workforce systems.
...
1. **Entity + terminology + granularity** — the practice's word is **manning
   board / people deployment / labor line-up**; SAP DM calls the record a
...
4. **Does scheduling consume it?** — in ERP/MES cores and WFM: **no** (the
   ...
   maintains, not from a daily people board. SAP DM is the only place the shift
...
### Epicor Kinetic
No people entity. Shop Employee carries a *default* Resource Group/Resource
...
MES. People size on operations is an anonymous duration/cost multiplier.
...
1. **New entity: `peopleAssignment`** — `(id, companyId, locationId,
...
2. **Manager UI: a "People" board in the Schedule area** (sibling view to
...
4. **Scheduler consumption (v2, the differentiator):** for dates with
   ...
   at each work center to that day's assigned people (Asprova's "resource valid
   condition" semantics — eligibility, not a new algorithm). Fall back to
   any-qualified-on-shift where no people is defined (config: hard vs soft).
   People changes fire `schedule.inputs.changed` (new kind `"people"` with
...
5. **Terminology:** call the surface **People** (board), the row a **people
   assignment** — matches industry speech ("people deployment", "manning")
...
### Epicor
- https://www.epiusers.help/t/resource-group-with-people-size/81399
