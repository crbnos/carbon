# Copy-Week (People Schedule) Research: Best Practices Survey

## Summary

Surveyed how workforce-scheduling and manufacturing tools let a supervisor copy a
people/shift schedule from one period onto another. The headline finding contradicts
...
- **Manufacturo**, **First Resonance ION** — MES peers; surveyed and found to have
  **no** people copy-schedule feature (negative result, see below).
...
- **Rationale**: a recurring people pattern is a reusable object, not a one-off copy.
...
Neither MES has a people copy-schedule feature. Manufacturo publishes no
workforce-scheduling module (its `Dispatching` is assign-work-now, not plan-a-week).
ION schedules *runs and steps*, not people; its nearest analogue is **Run Batching**,
...
Carbon's `copyPeopleWeek(fromWeekStart, toWeekStart)` already accepts arbitrary source
...
2. **Add a repeat count** (Microsoft Shifts' pattern): "Repeat ___ times", default 1.
   This covers "people the next 4 weeks the same" in one action and is the standard
...
5. **Explain skips rather than hiding them.** Deputy's silent drop is a documented
   pain point. When the preview shows skips, say why ("3 already assigned, 1 on time
...
7. **Defer templates, but design for them.** Templates are the consensus answer to
   ...
   `peopleWeekTemplate`). Not needed for this change, but the copy dialog's shape should
   not foreclose it.
8. **Do not adopt a draft/publish model** just for copy. It is how competitors buy
   safety, but it is a large architectural addition to Carbon's people board; the
...
- `copyPeopleDayInTransaction` and `assignPeopleWeek` built their skip sets from
  `companyId + date` with **no `locationId`**, so a person assigned at another site was
  silently dropped from the copy.
- Worse, `upsertPeopleAssignment` and `setPeopleDay` ran **DELETEs** without a
