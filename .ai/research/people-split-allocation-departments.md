# Split Allocation, Overtime & Department-Scoped People Scheduling: Best Practices Survey

> Follow-up to `.ai/research/daily-people-station-assignment.md` and
> `.ai/research/work-center-capacity-people-matrix.md`. Questions from Naveen:
...
- **PlanetTogether Attention Percent**: multi-tasking resources; each operation
  ...
  concurrent ops. Fractional people (0.5 person) unverified (docs gated).
...
1. **Department as a top-level, persisted filter (build first — no schema
   change).** `?department=` URL param on the people page applied to ALL views
...
2. **One `hours` column on `peopleAssignment` covers BOTH split allocation and
...
3. **Saturday people already works** — assigning people on a weekend replaces the
   0-hour calendar fallback with people hours; document it as the "add a Saturday
...
1. **Schema** (one migration): add `hours NUMERIC(5,2) NULL` to
   `peopleAssignment` (null = the whole shift — today's behavior, so existing
...
2. **Split UX**: drag keeps meaning MOVE (moves that assignment row). Splitting
   ...
   this is the largest code change (PeopleBoard drag ids, optimistic moves,
...
- Popover-over-modal criterion (Apple HIG / hidde.blog): use a popover when
  seeing the underlying context while choosing matters — on the people board,
