...
| Board visibility of `Completing` | RPC returns `Active` + `Completing` batches; `Completing` lanes render **read-only** (yellow badge, no drop targets, no dissolve, link to the MES batch page) | Durable `Completing` exists so failures wait visibly for a human — hiding them from the planning surface would undercut it. Drag/dissolve stay gated to `Active` |
