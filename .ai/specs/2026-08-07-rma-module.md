...
| Quality relationship | RMA escalates TO an Issue (in scope) | Issue→return creation is a **follow-on spec**; v1 creates supplier returns standalone |
...
- ~~Issue ↔ supplier-return linkage~~ — moved to the follow-on bridge spec (association junction, enum value, and ownership rules travel with it).
...
- **Quality bridge — deferred to a follow-on spec** (PR #1354 review: *"it does make sense to me to create RMAs from NCRs, but let's do that in a follow-on spec"*). v1 ships the standalone creation path only, and the Issue's existing direct write-off in `post-nonconformance` is unchanged. The follow-on spec owns the whole bridge: the **Create Supplier Return** action on an Issue, supplier resolution (single `nonConformanceSupplier` else receipt-parent else explicit pick), the `nonConformancePurchaseReturnOrderLine` association with per-quantity ownership, idempotent drafting, and the Issue-close block that makes write-off = `issue qty − shipped-via-return qty`. Until it lands, an Issue write-off and a manually created supplier return for the same goods are **unlinked** — exactly today's behavior — so process discipline is: pick one path per lot.
...
| With the bridge deferred, an Issue write-off + a manually created supplier return for the same goods can double-relieve inventory | Med | Unchanged from today's unlinked behavior; the follow-on bridge spec adds per-quantity ownership + the Issue-close guard — until then, one path per lot by process |
...
- [x] Supplier-side shape? — **Review (PR #1354, changes requested: "Let's do the purchasing side as part of the spec too"):** included as a direction-flipped mirror — authorize → **ship** → credit (NetSuite Vendor RA order of operations), shared `returnReason` table (BC shares the reason codes across sales and purchase returns), no disposition stage (goods leave; nothing to disposition), and the supplier's own RMA number on the header (`supplierReference`). See the Purchasing Side section. (The Issue→return bridge originally included here was later deferred — next question.)
- [x] Create return documents from NCRs/Issues? — **Review (Slack, 2026-08-13): "it does make sense to me to create RMAs from NCRs, but let's do that in a follow-on spec."** Deferred: v1 creates supplier returns standalone only; the RMA→Issue escalation direction (scrap/rework GL) stays in scope. The follow-on spec owns the whole bridge — the Create Supplier Return action, supplier resolution, `nonConformancePurchaseReturnOrderLine` per-quantity ownership, idempotent drafting, and the Issue-close guard — plus any future NCR→customer-RMA creation.

## Changelog

- 2026-08-13: Issue→return bridge carved out to a follow-on spec per review on PR #1354 ("it does make sense to me to create RMAs from NCRs, but let's do that in a follow-on spec"): Create Supplier Return, supplier resolution, `nonConformancePurchaseReturnOrderLine` + its `nonConformanceAssociationType` value, per-quantity ownership, idempotency, and the Issue-close block all move to the follow-on. The RMA→Issue escalation (scrap/rework GL via `post-nonconformance`) remains in scope. TLDR, deltas table, data model, flows, acceptance criteria, and risks updated.
