...
Draft ──confirm──▶ Confirmed ──activity──▶ In Progress ──complete──▶ Completed
  └──cancel──▶ Cancelled (only while nothing received)      (guard: every line Shipped or
                                                             Scrapped, or short-closed
                                                             while still Pending)
...
1. **Register.** The stamping event follows the term's `startBasis`: **Ship Date** terms register when the sales shipment posts (`post-shipment`); **Invoice Date** terms register when the sales invoice posts (`post-sales-invoice`, resolved to the invoiced shipment lines — NetSuite's invoice-triggered registration). Either way the insert runs **inside the posting transaction** (a failure rolls the posting back — a posted shipment without its registrations is not a reachable state), creating one row per shipped tracked entity (serial/batch) or one quantity row per shipment line for untracked items (NetSuite's lot pattern). Every auto-created row persists its **stamping-document line key** — ship-stamped rows their `shipmentLineId`, invoice-stamped rows their `salesInvoiceLineId` (plus `shipmentLineId` when resolvable, as lineage) — which is both the void key and the idempotency key: unique per stamping line + entity, so a void/re-post cycle cannot duplicate rows, and each registration is removed exactly when its **own stamping document** is voided. Dates resolve from the term into per-class expirations (`partsExpirationDate` / `laborExpirationDate` — BC's split). For tracked units whose genealogy reaches a purchase receipt and whose item has a `supplierWarrantyTermId`, the same row stores `supplierId` + `supplierWarrantyExpirationDate` — the data that answers "is the OEM leg free for us?" (Acumatica Company/Vendor warranty pair; SyteLine warranty Type). Registrations are editable per unit (override), creatable manually (legacy/imported units), and voided with the shipment. Each registration has a readable id (`WTY000001`) and a **warranty certificate PDF** — the "warranty card" a supplier hands the customer; no surveyed ERP ships one.
2. **Check.** Creating an RMA line or a repair order line for a registered unit surfaces the per-class verdict inline (advisory — BC/SAP B1/Epicor behavior; NetSuite's hard block is the rejected outlier, it forces workarounds for goodwill repairs). Tracked units resolve by `trackedEntityId` — and when one entity carries several registrations (a resold/re-shipped unit, manual rows), the verdict prefers a registration that **still covers the unit today** and only then takes the **latest `startDate`** (ties: latest `createdAt`, then highest `id` — a total order even for bulk-inserted rows). Recency alone would let an expired-but-newer row shadow an original that is still live. Intake persists that pick on the line like any other. Untracked items can match several registrations across repeat purchases, so the check is registration-specific: intake lists the customer's candidate registrations for the item and the user picks one (or explicitly none — unregistered), and the chosen `warrantyRegistrationId` is stored on the repair line as the resolved identity. Two guards keep that reference honest: the registration must **match the line** (same `itemId`, the order's `customerId`, and the line's tracked entity when it holds one) — checked at line save and re-checked by the `issue` function before any charge posts as covered; and an untracked registration's `quantity` is a **cap** — saving a line that references one re-checks, under a row lock on the registration, that the summed quantity of open repair lines referencing it stays within the registered quantity (a deleted or cancelled line releases its share automatically, since the cap is derived, never stored).
...
8. **Return to customer.** A shipment with `sourceDocument 'Repair Order'` ships `'Repaired'` lines home: entity → `Consumed` (back "at customer"), line → `'Shipped'`. On completion, an optional **Apply Repair Warranty** action creates a **new** registration for the unit from the completion date with the chosen term (BC's replacement-warranty-reset precedent, generalized). It is manual-class — no stamping keys, so a later void of the original shipment or invoice can never delete it — carries `repairOrderLineId` as provenance, and leaves the original registration untouched; the latest-`startDate` selection rule makes the repair warranty govern from then on while the history stays intact.
...
| Repair warranty | Optional **Apply Repair Warranty** action at completion — creates a NEW manual-class registration (no stamping keys, `repairOrderLineId` provenance) dated from completion; the original row is never mutated | BC component-replacement reset is the only documented precedent; per-action picker, no company default column. A new row survives source-document voids and preserves warranty history; the latest-`startDate` rule makes it govern |
...
**Every relationship below is a composite, company-scoped FK** — `FOREIGN KEY ("x", "companyId") REFERENCES parent("id", "companyId")` — so an id can never point into another tenant, with the delete action naming its column where needed (`ON DELETE SET NULL ("x")`, the composite-FK lesson's PG15 column-list form). Referenced parents (`item`, `customer`, `supplier`, `trackedEntity`, `shipmentLine`, `salesInvoiceLine`, `salesReturnOrder(Line)`, `quote`, `salesOrder`, `purchaseOrder`, `returnReason`, `warrantyTerm`, `warrantyRegistration`, `location`) already carry — or get, in this migration where missing — a `(id, companyId)` unique key (`salesInvoiceLine` is a known gap: its table has a bare `PRIMARY KEY ("id")`, so this migration adds its unique key). The stamping-key FKs are `ON DELETE SET NULL` safety nets only — registration cleanup on void is performed **explicitly by the posting functions**, because a shipment or invoice void reverses status without deleting its line rows, so no FK action can do that job. The SQL sketches show business columns; FK clauses follow this rule throughout. One deliberate exception: user references (`assignee` and the audit columns) stay plain `REFERENCES "user"("id")` — the house pattern on every existing table, including the returns module — with company membership enforced by RLS and the application layer, not by a `userToCompany` composite FK.
...
CREATE TYPE "repairOrderLineStatus" AS ENUM ('Pending', 'Received', 'At Supplier', 'Repaired', 'Shipped', 'Scrapped');
...
CREATE TABLE "warrantyRegistration" (
    ...
    "shipmentLineId" TEXT,                            -- stamping key on ship-stamped rows; lineage-only on invoice-stamped rows when resolvable
    "salesInvoiceLineId" TEXT,                        -- stamping key on invoice-stamped rows; NULL on ship-stamped and manual rows
    ...
    "repairOrderLineId" TEXT,                         -- provenance of a repair-generated warranty (SET NULL); NULL otherwise
    "companyId" TEXT NOT NULL,
...
Indexes on `companyId`, `customerId`, `itemId`, `trackedEntityId`. Idempotency is a constraint, not a convention — every auto-created row is unique per **its stamping-document line** (ship-stamped rows key on `shipmentLineId`, invoice-stamped rows on `salesInvoiceLineId`):

```sql
CREATE UNIQUE INDEX "warrantyRegistration_ship_tracked_idx" ON "warrantyRegistration"
  ("shipmentLineId", "trackedEntityId", "companyId") WHERE "salesInvoiceLineId" IS NULL AND "shipmentLineId" IS NOT NULL AND "trackedEntityId" IS NOT NULL;
CREATE UNIQUE INDEX "warrantyRegistration_ship_untracked_idx" ON "warrantyRegistration"
  ("shipmentLineId", "companyId") WHERE "salesInvoiceLineId" IS NULL AND "shipmentLineId" IS NOT NULL AND "trackedEntityId" IS NULL;
CREATE UNIQUE INDEX "warrantyRegistration_invoice_tracked_idx" ON "warrantyRegistration"
  ("salesInvoiceLineId", "trackedEntityId", "companyId") WHERE "salesInvoiceLineId" IS NOT NULL AND "trackedEntityId" IS NOT NULL;
CREATE UNIQUE INDEX "warrantyRegistration_invoice_untracked_idx" ON "warrantyRegistration"
  ("salesInvoiceLineId", "companyId") WHERE "salesInvoiceLineId" IS NOT NULL AND "trackedEntityId" IS NULL;
```

Auto-created rows are deleted when their **own stamping document** is voided — ship-stamped rows matched on `shipmentLineId`, invoice-stamped rows on `salesInvoiceLineId`; manual rows (both keys NULL) survive.
...
The one-shot link actions (Create Quote / Create Sales Order / Create Repair PO) run as a **row-locked check-then-create in one transaction**: lock the `repairOrder` row (`SELECT … FOR UPDATE`), re-read the link, return the already-linked document if set, else create the child and set the link before commit — so two concurrent requests cannot each mint a child and race on the pointer (the second finds the first's link under the lock and returns it). These partial uniques are the backstop that keeps even a bug from double-linking, not the mechanism:
...
CREATE UNIQUE INDEX "repairOrderLineTrackedEntity_one_per_line_idx" ON "repairOrderLineTrackedEntity"
  ("repairOrderLineId", "companyId");  -- at most ONE entity per line, by constraint not convention
```

`quantity` is 1 for serials and equals the line quantity for a batch entity (one batch, N units) — validated at write; the one-row constraint is what keeps the line's scalar custody state truthful.
...
- Repairs: `getRepairOrders`, `getRepairOrder`, `getRepairOrderLines`, `getRepairOrderCharges`, `upsertRepairOrder(Line/Charge)`, `deleteRepairOrder(Line/Charge)`, `confirmRepairOrder`, `cancelRepairOrder`, `completeRepairOrder` (guard: **at least one line exists**, and every line is `Shipped` or `Scrapped`, or `closedComplete` — an empty repair order cannot complete, and short-close is permitted **only while a line is still `Pending`**: once a unit is in custody it must exit through a ship-back or a scrap, never vanish behind a flag), `markRepairOrderLineRepaired`, `scrapRepairOrderLine` (for abandoned/unrepairable units, in custody states `Received`/`Repaired`, always the full line quantity: tracked lines flip the held entity `Scrapped` via the existing entity-scrap flow; untracked lines post the quantity-scrap path — negative `itemLedger`, `documentType 'Scrap'`, at carried (zero) value — either way no GL, since the intake layer was zero-value, and the line's custody goes terminal `'Scrapped'`), `applyRepairWarranty` (INSERTS a new manual-class registration — same item/customer/entity, `startDate` = completion date, dates from the chosen term, `repairOrderLineId` provenance — never mutating the source registration), `createRepairOrderFromReturnLine` (idempotent on `salesReturnOrderLineId`; copies customer/entities/reason; line starts `'Received'`; sets the RMA line disposition `'Repair'`), `createRepairQuote` (draft `quote` from Billable charges; idempotent on `quoteId`), `createRepairSalesOrder` (idempotent on `salesOrderId`).
...
- **`post-sales-invoice`** — the same, for terms with `startBasis 'Invoice Date'`: registrations stamp from the posted invoice date, every row keyed on its `salesInvoiceLineId` (with `shipmentLineId` carried as lineage when the invoice line resolves to a shipment line). The invoice-line-only fallback applies to **untracked items only** — a tracked line that cannot resolve to a shipment line has no source for its entities, so it is skipped rather than registered with a null identity (manual registration is the recovery). Voiding the invoice removes the rows it created, matched on `salesInvoiceLineId`.
- **`issue`** — `partsToRepairOrder` case: re-validates the line's registration identity (item/customer/entity match) before accepting a `Warranty`/`No Charge` code, then posts the Part charge's consumption — negative `itemLedger` `'Repair Consumption'` at current cost, journal `Cr Inventory / Dr (warrantyCostAccount ?? costOfGoodsSoldAccount)` for `Warranty`/`No Charge`, `Dr costOfGoodsSoldAccount` for `Billable` (gated on `accountingEnabled`); stamps `unitCost` + `issuedAt` on the charge.
...
- [ ] An `'Invoice Date'`-basis term does NOT register at shipment posting; it registers when the sales invoice posts, dated from the invoice and keyed on `salesInvoiceLineId` (unique — re-posting after a void cannot duplicate), and voiding the invoice removes exactly those rows.
...
- [ ] Shipping a `'Repaired'` line back to the customer flips it `'Shipped'` (entity `Consumed`); Complete is blocked while any line is neither `'Shipped'`, `'Scrapped'`, nor short-closed; Cancel is blocked once anything is received.
- [ ] Short-closing is refused on any line past `'Pending'`; an abandoned in-custody unit is resolved via Scrap Unit (entity `Scrapped`, zero-value so no GL, custody terminal `'Scrapped'`), which unblocks Complete — a held unit can never disappear behind a flag.
- [ ] Registrations resolve deterministically for a tracked entity with multiple rows (resale/manual): the latest `startDate` wins (ties broken by `createdAt`, then `id`) and the pick is stored on the repair line.
- [ ] A repair line referencing a registration whose item, customer, or entity doesn't match is rejected at save and again at charge issue; two repair lines cannot jointly claim more units than an untracked registration's quantity (row-locked cap).
- [ ] Completing a repair order with zero lines is refused.
- [ ] Apply Repair Warranty inserts a NEW registration (provenance = the repair line) and voiding the original sales shipment afterwards deletes only the original registration — the repair warranty survives.
- [ ] An Invoice-Date-basis TRACKED item whose invoice line cannot resolve to a shipment line does not auto-register (no null-identity rows); the untracked fallback still registers.
- [ ] Scrapping an untracked repair line writes the zero-value quantity scrap ledger row and the line goes `'Scrapped'`, exactly as a tracked line's entity-scrap does.
...
- 2026-08-28 (b): **Warranty terms decoupled from the part** (customer feedback: the same battery needs two years for one customer and one for another). A term on the item is now only a DEFAULT. Resolution at stamping, most specific first: the shipment line's own `warrantyTermId` (the shipper's override on the day) → a `customerWarrantyTerm` rule for that customer + item → that customer's catch-all rule (`itemId IS NULL`) → `item.warrantyTermId` → none. Adds the `customerWarrantyTerm` table (per-entity assignment, not a group×group matrix), `shipmentLine.warrantyTermId`, a Warranties panel on the customer, a per-line warranty picker on sales shipments, `resolveWarrantyTermForSale` so the UI can show which rule wins, and the warranty-registration edit screen that was specified but never built — the registration remains the truth, and any date on it can be corrected per unit.
- 2026-08-28: Review round on the implementation PR (#1497): coverage selection now prefers a registration still active today before falling back to recency; the repair cost layer sets `remainingQuantity` (a zero-value layer with 0 remaining is never consumable, so a ship-back would have consumed a valued layer); part issuing claims the charge inside its transaction and the coverage re-validation fails closed; repair receipt/shipment draft reuse is leg-aware; warranty-term writes require the employee role; repair warranties are refused on settled orders.
- 2026-08-27 (d): Third review round on PR #1496, all eight findings accepted: (1) `completeRepairOrder` refuses an empty order; (2) registration selection tie-break extended to `id` (total order); (3) untracked registration `quantity` is a row-locked cap across open repair lines; (4) `salesInvoiceLine` added to the composite-parent contract (its missing `(id,companyId)` key created in the migration) and stamping-key FKs clarified as SET NULL safety nets with cleanup owned by the posting functions; (5) registration↔line identity (item/customer/entity) validated at line save and re-validated by `issue` before covered charges post; (6) `applyRepairWarranty` now INSERTS a new manual-class registration with `repairOrderLineId` provenance (new column) instead of mutating the void-owned source row; (7) untracked scrap path defined (zero-value quantity scrap ledger row, full quantity); (8) invoice-line-only registration fallback restricted to untracked items — tracked lines without shipment resolution are skipped, never registered with a null identity.
- 2026-08-27 (c): Second review round on PR #1496 — five findings accepted, one declined with reason: (1) invoice-stamped registrations get their own stamping key `salesInvoiceLineId` (+ per-key partial unique indexes and invoice-void matching; the "voided with the shipment" wording generalized to the stamping document); (2) deterministic tracked-registration selection — latest `startDate` wins, pick persisted on the line; (3) link actions specified as row-locked check-then-create in one transaction (unique indexes demoted to backstop); (4) one tracked entity per line enforced by a unique index on the junction (batch quantity semantics stated); (5) short-close restricted to `'Pending'` lines, with a terminal `'Scrapped'` custody state + `scrapRepairOrderLine` via the existing scrap flow so an in-custody unit can never vanish behind `closedComplete`. Declined: composite FKs from user columns to `userToCompany` — every existing table (returns included) references global `user(id)`; the exception is now documented in the FK paragraph.
