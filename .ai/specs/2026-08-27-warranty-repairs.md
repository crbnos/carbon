...
- Repairs: `getRepairOrders`, `getRepairOrder`, `getRepairOrderLines`, `getRepairOrderCharges`, `upsertRepairOrder(Line/Charge)`, `deleteRepairOrder(Line/Charge)`, `confirmRepairOrder`, `cancelRepairOrder`, `completeRepairOrder` (guard: every line `Shipped` or `Scrapped`, or `closedComplete` — and short-close is permitted **only while a line is still `Pending`**: once a unit is in custody it must exit through a ship-back or a scrap, never vanish behind a flag), `markRepairOrderLineRepaired`, `scrapRepairOrderLine` (for abandoned/unrepairable units: the existing ERP scrap flow flips the entity `Scrapped` — zero-value, so no GL — and the line's custody goes terminal `'Scrapped'`), `createRepairOrderFromReturnLine` (idempotent on `salesReturnOrderLineId`; copies customer/entities/reason; line starts `'Received'`; sets the RMA line disposition `'Repair'`), `createRepairQuote` (draft `quote` from Billable charges; idempotent on `quoteId`), `createRepairSalesOrder` (idempotent on `salesOrderId`), `applyRepairWarranty` (stamp a term onto the registration from completion date).
...
- **`post-sales-invoice`** — the same, for terms with `startBasis 'Invoice Date'`: registrations stamp from the posted invoice date, every row keyed on its `salesInvoiceLineId` (with `shipmentLineId` carried as lineage when the invoice line resolves to a shipment line; an unresolvable line still registers, keyed on the invoice line alone). Voiding the invoice removes the rows it created, matched on `salesInvoiceLineId`.
...
- [ ] Registrations resolve deterministically for a tracked entity with multiple rows (resale/manual): the latest `startDate` wins and the pick is stored on the repair line.
