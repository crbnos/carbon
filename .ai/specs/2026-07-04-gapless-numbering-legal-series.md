...
- 2026-07-24: Foundation implemented (migration `20260721162233`): `sequence` hardening + immutability trigger, atomic allocators (`get_next_sequence_atomic` / `get_next_legal_series_number`), `legalSeries` table + RLS + trigger, nullable draft number columns behind status CHECKs, per-company `gaplessFrom` cutover; shared edge `getNextSequence` made atomic. `feature/gapless-numbering` adds: allocator REVOKE hardening (`20260724161500`), `legalSeries` service CRUD + settings UI, sequence format-lockdown UI, and the `getDocumentReadableId` draft-placeholder primitive. Deferred to the coordinated posting-time wave (Decision 15, DB-gated): moving the six draft-creation allocators to posting time, the `get_next_sequence` RPC guard, placeholder render-site wiring, and #1047's audit coverage / `isBackdated` export. Full call-site map in `.ai/runs/1038/status.md`.
