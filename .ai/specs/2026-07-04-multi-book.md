...
- 2026-07-29: **Schema foundation shipped** (PR for #1052, branch `feature/multi-book-adjustment-books`). Migration `20260729141133_multi-book-adjustment-books.sql` creates the full data model: the shared #1047 book DDL (`accountingBook`, `accountingBookCompany`, `journal.bookId` + backfill + `journal_default_book` resolver trigger), the `journal_adjustment_book_guard` guardrail trigger, `journalEntrySourceType += 'Book Adjustment'`, `fixedAssetBook`, `depreciationRun.bookId`, and `bookAdjustmentRun` — all idempotent (`IF NOT EXISTS` / `DROP POLICY`-then-`CREATE`), RLS per conventions (group-scoped helpers on `accountingBook`, company-scoped on the rest), seeded (Primary US-GAAP book per group, enabled per company via `'system'`). Pure-TS models added to `accounting.models.ts` (`accountingBookTypes`, `accountingPrinciples`, `bookModes`, `bookAdjustmentRunStatuses`, and the four validators). Two implementation notes vs the sketch: (1) `journal.bookId` is left **NULLABLE** rather than `SET NOT NULL` — a static NOT NULL would fail deploy on any company with `companyGroupId IS NULL` (the FK is `ON DELETE SET NULL`); the resolver trigger stamps Primary on every insert and reporting treats NULL as Primary. (2) The guardrail compares `"sourceType"::text` against text literals (never an enum literal), so the migration is safe even under a transaction-wrapping runner. **Deferred to a follow-up PR** (they consume the new tables through Kysely/supabase-js and therefore require regenerated `@carbon/database` types, which need a live DB + #1047 reconciliation — unavailable in this environment; hand-editing generated types is prohibited): book CRUD/enablement service functions, the `p_book_id`/`p_book_mode` RPC parameters and their callers, the `accounting.server.ts` generator registry + `runBookAdjustmentGenerator` + `statutory-depreciation` generator, close-checklist task registration + `book-balanced` auto-check, audit-log coverage (`audit.config.ts` entities + `attach_event_trigger` calls), and all UI (Books settings page, JE book selector, report book picker/mode toggle, fixed-asset Books section).
