...
| Member counts | `userCount` = direct active member users; `groupCount` = direct child groups. Counts shown plainly (no parentheses). | Direct counts are one indexed aggregate; recursive counts would reintroduce the recursive walk. EmailRecipients' badge becomes direct-count (was recursive unique-email count) — accepted trade-off; emails still resolve recursively on selection. |
...
| User search scoping | Users are matched via `ilike fullName` **scoped to the company through `userToCompany`** (inner-join filter), `active = true`, limit 20 | Today's `users.search.tsx` relies on `user`-table RLS alone with no explicit company filter — the rewrite adds defense-in-depth per the multi-tenancy lesson. |
...
- **Browse mode** (input < 2 chars): paged top-level groups. Group row = name + `userCount` badge (plain number) + chevron. **Row click selects the group** (adds/removes like any option); **chevron click expands** without selecting; hover prefetches members. Expanding renders direct users and child groups; child groups behave identically (selectable + expandable) at every depth. In `usersOnly` mode group rows aren't selectable — row click expands; groups with `userCount = 0 AND groupCount = 0` are hidden.
