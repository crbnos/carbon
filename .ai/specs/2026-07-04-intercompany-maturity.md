...
- 2026-08-01: WS4 netting math kernel (DB-independent slice, pulled forward ahead of type regen — same split as the schema foundation). `computeNettingPosition` (net = min of the two mutual gross balances; residual + net-debtor payer) and `allocateNettingApplications` (oldest-first per-invoice `appliedAmount`, cents-tied, reports the unapplied shortfall) added to `accounting.utils.ts` with 11 TDD unit tests. Consumed by the deferred `createNettingStatement` snapshot precompute and the paired-payment settlement path; no schema, RLS, or query surface. Branch `feat/intercompany-netting-math-1058`.
