...
- 2026-07-24: **Phase 1 implemented** (`20260724100357_intercompany-maturity-matching.sql`) — workstreams 1 (tolerance matching) and 2 (FX differences). Delivered: `companyGroup.intercompanyMatchingTolerance` (default 1.00, `>= 0` check) + `intercompanyDocumentMirroring` flag; `intercompanyTransaction.documentAmount`/`matchedDifference`/`differenceKind`; `accountDefault.intercompanyDifferenceAccount` (by id) with a per-group `7095 Intercompany Differences` account seeded + backfilled (left NULLABLE so an unset account raises, per AC3); a three-pass `matchIntercompanyTransactions` (exact → FX doc-currency equality → same-base-currency greedy tolerance via a single-use greedy loop rather than a chain-prone set-based dedup); and a `generateEliminationEntries` difference plug that posts the **journal residual** to the difference account (balances by construction; covers both FX and tolerance; exact matches skip it). While rewriting `generateEliminationEntries` for the plug, fixed three inherited latent crash bugs that had left the elimination path non-functional: a phantom `journalLine.companyGroupId` insert (column doesn't exist — also removed from `createIntercompanyTransaction`), `v_journal_id INTEGER` vs the TEXT `journal.id`, and a missing NOT-NULL `journalEntryId` (now sequence-assigned). Validators added to `accounting.models.ts` (`intercompanyMatchingSettingsValidator`, `intercompanyDifferenceKinds`). **Deferred to follow-up PRs** (blocked on a running stack for type regen + posting verification the build env lacked): workstream 3 (document mirroring — Inngest job, cross-company writes, item-reference table, UI) and workstream 4 (netting workbench — `settleNettingStatement` RPC, paired payments, UI). Their shared columns (`intercompanyDocumentMirroring`, `documentAmount`) land here; their tables do not. Population of `documentAmount` across posting paths (required to exercise pass 2) is likewise follow-up. Spec stays `in-progress`.
- 2026-07-25: **Workstream 4 (netting) — data-model foundation** (`20260725143012_intercompany-netting-foundation.sql`). Lands the schema only, deliberately ahead of the consuming RPCs/service/UI because those reference the new tables in TypeScript and need regenerated `@carbon/database` types (a running local stack the build env still lacks). Delivered: `accountDefault.intercompanyNettingAccount` (by id) with a per-group `1135 Intercompany Netting Clearing` current-asset account seeded (sibling of 1130, parented under the `Receivables` group header resolved by isGroup+name+class) + backfilled, left NULLABLE so the settlement path raises rather than posting to the wrong clearing account; `intercompanyNettingStatement` + `intercompanyNettingStatementLine` (group-level, single-column PK + `companyGroupId`/company columns per the `intercompanyTransaction` precedent, `currencyCode` a bare TEXT with no FK for the same reason); `payment.nettingStatementId` back-reference + partial index; an `intercompanyNettingStatement` readable sequence (`ICNS-%{yyyy}-`) backfilled for existing companies + seeded for new; and RLS mirroring the `intercompanyTransaction` source-OR-target shape (`accounting_create/update/delete`), with the line policies gating on the parent statement's two companies via `EXISTS` (fully-qualifying the line's `statementId` to avoid ambiguity with the statement's readable-number column of the same name). `seed.data.ts` updated (1135 account, `intercompanyNettingAccount` default, netting-statement sequence). **Still follow-up** (needs the stack for type regen + posting verification): the statement-lifecycle RPCs (`createNettingStatement`/propose/agree/cancel) and settlement — note settlement rides the existing `post-payment` **edge function** (staged Draft payments + `invoiceSettlement` rows, invoked per company), not a self-contained RPC as the spec's §"RPC changes" sketch implied; that gap should be resolved when the settlement path is built. Plus the whole matrix/statement/mirroring UI and workstream 3.
- 2026-07-27: **Workstream 3 (document mirroring) — data-model foundation** (`20260727061442_intercompany-document-mirroring-foundation.sql`). Schema only, again ahead of the consuming Inngest job / service / UI because those reference the new tables in TypeScript and need regenerated `@carbon/database` types (the build env has no local Postgres stack). Delivered: `intercompanyDocumentLink` (group-level single-column PK + `companyGroupId`/source+target company columns per the `intercompanyTransaction` precedent; polymorphic `sourceDocumentType` ∈ {`purchaseOrder`,`salesInvoice`} → `targetDocumentType` ∈ {`salesOrder`,`purchaseInvoice`}, bare-TEXT document ids with no FK; `status` machine `Pending`→`Mirrored`/`Failed`/`Exception`/`Detached`; `failureReason`, `lastSyncedAt`; unique on the source document; `createdBy` nullable for job-created rows) and — resolving Open Question 5 down its ambitious branch — the explicit `intercompanyItemLink` cross-company item reference table (directional source→target item mapping, `matchMethod` ∈ {`ReadableId`,`Manual`}), **seeded by auto-matching items across every ordered pair of companies in a group on the true identity tuple `(readableId, revision, type)`** (not the lossy generated `readableIdWithRevision`, which could alias two distinct items into a silent wrong-match; `item_unique` makes the tuple unique per company, so no fan-out) — both directions, so PO→SO and invoice mirroring both resolve; idempotent via `ON CONFLICT`. RLS on both tables mirrors the `intercompanyTransaction`/netting source-OR-target shape (`accounting_create/update/delete`). No account/sequence seed is needed for this workstream, so `seed.data.ts` is untouched. **Still follow-up** (needs the stack for type regen + runtime verification): the `intercompany.mirror.requested` Inngest job (PO release → partner Draft SO, sales-invoice post → buyer Draft purchase invoice), the one-way re-sync + loop-guard + failure-surfacing logic, the `getIntercompanyDocumentLinks`/`retryIntercompanyMirror`/`detachIntercompanyLink` service functions, and the Mirroring UI tab. Spec stays `in-progress`.
