...
**Decision (2026-08-15, IMPLEMENTED).** Option B is done: the tables are merged into one `enforcementRule` table discriminated by `family` ('storage' | 'sales'), with `enforcementRuleItemAssignment` (shared by both families), `enforcementRuleWorkCenterAssignment` (storage only) and `enforcementRuleAcknowledgment`. Migrations `20260815143022_enforcement-rules-table.sql` + `20260815143512_migrate-rules-into-enforcement-rules.sql`. It was done pre-merge of the sales feature so production performs exactly ONE table transition (the shipped `storageRule` → merged), with the unshipped sales tables folded in at the same time.
