...
The same restructuring fixes **module placement**: rule features are not their own ERP domains, so neither gets a standalone `~/modules/*` directory. Build item rules inside `~/modules/items` from the start (see "ERP placement" below); fold the existing standalone `~/modules/storage-rules` into `~/modules/inventory`, its domain module.
