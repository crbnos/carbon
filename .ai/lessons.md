...
## A no-DB loop worktree cannot build routes/services that reference new tables or columns — they can't typecheck without regenerated types

**Context:** Building the integration-surface spec (#1059) in a fresh `loop/<id>` worktree. The spec adds two tables (`apiIdempotencyKey`, `webhookRegistration`) and three `journal` columns, then wires REST routes/services against them. The worktree has no Supabase stack, so `pnpm db:migrate` / `pnpm run generate:types` can't run.

**Problem:** `packages/database/src/types.ts` is generated and must never be hand-edited. Until it's regenerated against the applied migration, `client.from("webhookRegistration")` or a reference to `journal.externalId` is a hard TS error, so any route/service touching new schema fails the floor-gate typecheck. You cannot deliver those layers green in a no-DB worktree — only the DB-independent substrate (pure zod validators, pure helpers, the Inngest handler signing over JSON/config, an OpenAPI doc, and routes over *existing* service functions) typechecks.

**Rule:** In a no-DB loop, split schema-dependent work off explicitly. Ship Phase 1 = everything provable without a DB (validators + pure helpers with red→green unit tests, handler logic, docs, routes composing existing typed services); write the migration as foundation but mark it unapplied; DEFER every route/service that reads a new table/column to Phase 2 (post-migration, after `generate:types`), and say so in the PR + OpenAPI. Keep the pure logic out of the DB-typed files so it stays unit-testable (e.g. `accounting.integration.ts`, `webhook-signing.ts`). Don't cast `.from("x" as any)` to force a green — the judge rejects it and it hides a real gap.

**Applies to:** any conductor/loop build that adds tables or columns and then consumes them (routes, services, Kysely queries); `.ai/rules/loop-worktree-no-deps-no-db` territory. Pairs with the memory note "Loop worktree has no deps/DB".
