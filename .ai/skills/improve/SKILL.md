...
- **Read the agent knowledge base** (these are Carbon's intent/design/ADR docs — treat decided tradeoffs here as by-design, not findings):
  ...
  - `.ai/scratch/tasks/lessons.md` — hard-won corrections (RLS pattern, upsert audit-field clobbering, `.merge()` after `.refine()`, `accountId` not `accountNumber`, enum conventions, etc.). **A finding that contradicts a lesson is by-design — do not report it.**
  ...
  - `.ai/scratch/recommendations/*.md` — existing forward-looking proposals (don't duplicate them in Direction).
...
- which `.ai/rules/` and `.ai/rules/conventions-*` files are relevant so the subagent reads Carbon's real conventions, plus the instruction that anything matching `.ai/scratch/tasks/lessons.md` is settled,
...
**Vet before presenting — subagents over-report.** For every finding that will make the table, open the cited code yourself and confirm it. Expect three failure classes: **by-design behavior** reported as a bug (e.g. a pattern that's actually the documented convention in `.ai/rules/conventions-*` or a settled call in `.ai/scratch/tasks/lessons.md`); **mis-attributed evidence** (real finding, wrong file or line); and duplicates across subagents. Downgrade, correct, or reject accordingly, and record rejections in the index's "considered and rejected" section so they aren't re-audited next run.
...
Present **direction findings separately**, after the table — 2–4 grounded suggestions max, each with evidence and trade-offs in two or three sentences. Cross-check `.ai/scratch/recommendations/` first so you don't re-propose what's already on file.

Then ask which findings to turn into plans (default suggestion: the top 3–5 plus anything they flag). Surface **dependency ordering** (e.g. "characterization tests for `@carbon/<pkg>` must land before the refactor"). Wait for the selection. Do not write 30 plans nobody asked for. If running non-interactively, write plans for the top 3–5 by leverage and record that default in `.ai/scratch/plans/improve/README.md`.
...
Finish by writing `.ai/scratch/plans/improve/README.md` with the recommended execution order, dependencies, and a status column.
...
- `next` (or `features`, `roadmap`) → run Recon, then audit only the direction category in more depth: 4–6 grounded suggestions, each with evidence, trade-offs, coarse effort. Selected ones become design/spike plans. Check `.ai/scratch/recommendations/` first.
...
- `review-plan <file>` → critique an existing plan in `.ai/scratch/plans/improve/` against the template's standards and tighten it. If you authored it this session, also have a fresh-context subagent read it cold and report ambiguities.
