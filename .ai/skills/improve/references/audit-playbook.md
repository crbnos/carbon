...
- Inconsistent patterns: multiple ways of doing data fetching / error handling / styling in one app — pick the winner the team converged on most recently (check `.ai/rules/conventions-*`) and plan the consolidation.
...
- **Agent knowledge gaps:** a subsystem with no `.ai/rules/` doc, or a convention not captured in `.ai/rules/conventions-*` — high leverage for an agent-driven repo. Recommend the rule/convention doc as a plan.
...
- Architectural decisions nobody can reconstruct for actively-contested areas (prefer adding to `.ai/rules/` over scattered docs).
- Stale docs that are actively wrong (worse than missing) — including stale `.ai/rules/` entries that no longer match the code.
...
- **Forms**: `ValidatedForm` + a zod validator + existing form components from `packages/react`/`apps/erp/app/components/Form`. Flag hand-rolled forms or actions without a validator (see `.ai/rules/conventions-forms.md`).
- **Service/models layout**: add to module-level `<module>.service.ts` / `<module>.models.ts`; flag new standalone service/models files (see `.ai/rules/conventions-services.md`).
