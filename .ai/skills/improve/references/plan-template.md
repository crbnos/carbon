...
Every plan is written for an executor model that has **zero context**: it has not seen the advisor session, the audit, the other plans, `.ai/rules/`, or any prior conversation. It may be a smaller/cheaper model. Assume it is competent at following explicit instructions and weak at filling gaps, recovering from ambiguity, or knowing when to stop.
...
- **The executor never runs database migrations/seeds/rebuilds** (`pnpm db:migrate`, `db:seed`, `db:types`, `crbn migrate`). If a plan adds a migration, the executor writes the SQL file per `.ai/rules/workflow-database-migration.md`; **the user applies it**. State this explicitly.
...
- The Carbon conventions that apply, **quoted inline** (the executor has not read
  `.ai/rules/conventions-*` or `.ai/rules/`), with a pointer to one exemplar file:
  "Service functions return `{ data, error }` and use Kysely transactions for
  multi-row writes — see `<exemplar>.service.ts` and match it."
- Any schema/migration constraints inlined from `.ai/rules/workflow-database-migration.md`
...
- Whether a `.ai/rules/` doc should be updated **after this is committed** (note it;
...
- Could a model that has never seen this repo or `.ai/rules/` execute this with only the plan file and the repo? If any step needs knowledge from the advisor session or the rules, inline it.
