...
**Date:** 2026-09-10 · **Status:** Parts A + B committed; Part C Ramp AND Rillet legs VERIFIED live 2026-09-10 (five real bugs found and fixed on the way — see "Part C findings"; uncommitted at time of writing); Xero leg optional/unverified; QBO unverifiable (no sandbox) · **Branch:** `feat/feat-ramp`
...
| D8 | Rillet Reimbursement object | **In scope.** Route Employee-supplier purchase invoices to `POST /reimbursements` (`payable_account_code` = the AP control account from the posted journal), always — no provider switch (a `reimbursementRepresentation` setting was built, then removed on Brad's 2026-09-10 review: an unnecessary choice; the native object is the only representation). **Caveat:** Rillet has no reimbursement-payment endpoint (only `POST /bills/{id}/payments`; a `ReimbursementPaymentRequest` schema exists in their spec with no path), so a Ramp-paid reimbursement stays `UNPAID` in Rillet and Carbon's payment push parks it `Skipped` with a visible reason. The email asks Rillet (a) when that endpoint ships and (b) whether their UI can mark one paid meanwhile; if both are "no", the customer marks the reimbursement paid in Rillet by hand until the endpoint exists. Xero/QBO reimbursements stay bills (that *is* their native shape). | Completeness; the object is what Rillet asked for; the payment gap is in Rillet's API and is surfaced, not hidden. |
...
## Part C findings (live, 2026-09-10)

**Ramp leg** — see the ticked items below. **Rillet leg** (sandbox, API key installed by Brad; fresh dev company `dag0l963i0gg2a8f2vjg`):

- [x] Hertz split charge → `POST /charges` `01a08df9-f68a-708b-9187-3889430b062e`: Hertz vendor (JIT), 2 items on 6090 (500 Engineering / 342.67 G&A via the auto-provisioned Cost Center Field), `charge_date` 07-21, `impact_date` 07-22, both `external_references`. Journal `je_6qqY…` → `Excluded / DOC_BACKED / charge`.
- [x] Void → `DELETE /charges/{id}` (GET → 404), mapping `{voided: true}`; the VOID journal → `Excluded / DOC_BACKED / charge` (after fix #3 below).
- [x] Delta refund (Credit): first (Rillet outside the set) charge op Skipped with the mirror-rule reason + journal pushed as a JE with per-line Cost Center fields; then a direct probe proved `POST /charges` accepts a negative item, Rillet was added to `CHARGE_CREDIT_PROVIDERS`, and the re-drive created the credit as charge `01a08e07-737e-73a2-b40a-fab849be2ff0` with items −200 (Engineering) and −59.49 (G&A) on 6090, Delta vendor, same dates; its journal → `Excluded / DOC_BACKED / charge`.
- [x] 5 statement Payments → 5 Rillet journal entries (backfill, `postingSync.syncFromDate` set).
- [x] Reimbursement: Employee-typed supplier + G/L purchase invoice AP000003 posted through `post-purchase-invoice` → `POST /reimbursements` `01a08dff-8c37-7039-9cfb-dbe25345b614` (`remoteKind: reimbursement`, UNPAID).
- [x] Payout: posted AP payment PAY-2026-09-000001 → Warning `UNSUPPORTED_REIMBURSEMENT_PAYMENT` with the "mark paid in Rillet" message.
- [x] Sync Activity tab renders "Card Charge <id>" rows (linked) across Completed/Skipped/Excluded/Failed/Warning.

Bugs found only by the live pass, all fixed + unit-tested:
1. Ramp CoA push leaked `visible` into the POST body (422) → `toRampGlAccountPayload`.
2. Ramp sync confirm sent empty lists / wrong failed-item shape (422, silent for weeks) → `buildSyncConfirmBody` + `confirmError` on step output.
3. **Void journal double-count**: the executor/planner keyed the backing card transaction on `cardTransaction.journalId`, which never matches the "VOID Card Transaction" journal, so the void pushed as a journal entry on top of the charge DELETE (Rillet netted −1 charge). → `loadCardTransactionPolicyInputs` resolves via journal-line `documentType/documentId` (shared by both call sites), fallback to `journalId`.
4. Ramp bill/reimbursement inserts had no `purchaseInvoiceDelivery` row, which `post-purchase-invoice` requires → `createPurchaseInvoiceDelivery` at both sites.
5. (Setting removed on Brad's review) `reimbursementRepresentation` — always the native object.

Customer preconditions surfaced (go in the email): map accounts (Match by code), and map the Ramp card-liability account to a Rillet **Credit Card**-subtype account (Rillet 400 otherwise, recorded as Failed with Rillet's message).
...
- [x] **Ramp sandbox (client-credentials, `environment: sandbox`)** — VERIFIED 2026-09-10: field `carbon-cost-center` present with the 4 cost centers as options; `GET /accounting/accounts?is_active=true` = 33 EXPENSE + 1 CREDCARD, all VISIBLE, nothing else (the August run's 79 stale accounts were deactivated first). Found + fixed a real bug on the way: the batch POST sent the Carbon-side `visible` flag → 422 DEVELOPER_7001. Note: a fresh DB against a business that already has a Carbon connection needs `metadata.connectionId` adopted by hand (`ensureRampConnection` only creates).: rename the CostCenter dimension to "Project", save settings → `GET /accounting/fields?remote_id=carbon-cost-center` returns the field with `name`/`display_name: "Project"`; `GET /accounting/field-options?field_remote_id=carbon-cost-center` lists every Carbon cost center with `id` = Carbon id. Add a cost center in Carbon → fire `ramp-sync` → it appears. Rename → `display_name` updates. `GET /accounting/accounts` shows only Expense accounts + the card liability `VISIBLE`, the rest `HIDDEN`.
- [x] VERIFIED — Hertz $842.67 coded via `PATCH /transactions/{id}` as a 2-line split (Ramp rejects single-line splits; `POST /accounting/ready-to-sync` marks it) → `CARD-2026-09-000006` Charge Posted, supplier "Hertz" (Card Merchant), lines 6090 + Engineering/G&A, `journalLineDimension` rows for both. Ramp returns the project selection as `type: OTHER` / `external_id: carbon-cost-center` — live proof of the read-back bug. Original bullet: `cardTransactionLine.costCenterId` set, `journalLineDimension` row present, `cardTransaction.supplierId` set to a "Card Merchant" supplier.
...
- [x] Regression: the 5 statement `Payment` transfers posted as journal-backed card transactions (Posted, journals present) — unchanged path. Also VERIFIED: the Delta refund → `CARD-2026-09-000007` Credit Posted with per-line projects; Ramp shows both Hertz and Delta `SYNCED` after the confirm-body fix (`buildSyncConfirmBody`: omit empty lists, `{ id, error: { message } }`). Receipts: Ramp's demo `receipt_url`s 404, so attach skipped by design (upload + `document` insert path probed OK).
