...
| ↳ trailing-zero preference | min 0 when set | `companySettings.hideCurrencyTrailingZeros` (default false) drops the non-significant zeros — "$300", "$3.5". DISPLAY and IN-APP only: `useCurrencyFormatter` and `MotionMoney` read it once via `useCurrencyMinDecimals`, never a call site. Documents render through the pure `formatMoney` and stay padded — a customer-facing invoice is where fixed-width money matters most — and an unauthenticated context (the public quote share page) has no settings to read, so it pads too |
