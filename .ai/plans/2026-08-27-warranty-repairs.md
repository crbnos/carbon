...
2. Write the SQL in this order, following the spec's **Data Model Changes** section exactly (spec is authoritative for columns; this task lists the deltas that are easy to get wrong):
   ...
   - **Additive columns:** `item.warrantyTermId TEXT`, `item.supplierWarrantyTermId TEXT` (both nullable, composite FK to `warrantyTerm`), `accountDefault.warrantyCostAccount TEXT` (nullable).
...
2. Lifecycle, each in a Kysely transaction with `SELECT ... FOR UPDATE` on the header (clone the returns row-lock pattern):
   ...
   - `completeRepairOrder` — guard: every line `'Shipped'` or `'Scrapped'`, or `closedComplete = true`; and `closedComplete` may only be set while the line is `'Pending'` (enforce in `upsertRepairOrderLine` too).
   - `markRepairOrderLineRepaired` — `'Received'` → `'Repaired'` only.
   - `scrapRepairOrderLine` — line must be `'Received'` or `'Repaired'`; flips the held entity to `'Scrapped'` using the SAME mechanism the ERP scrap flow uses (grep `"Scrapped"` writers in `apps/erp/app/modules/inventory` on the returns branch and reuse its service/edge call; the unit is at zero value so no GL). Sets custody `'Scrapped'`. If the existing scrap flow cannot be invoked from a service without a quality Issue, STOP and report — do not write a parallel scrap.
...
3. One-shot links, each row-locked check-then-create in ONE transaction per the spec (lock header, re-read link, return existing if set, else create child + set link before commit):
   ...
   - `applyRepairWarranty` — given a `warrantyTermId`, restamp the line's registration dates from the completion date.
...
**Steps:**
1. On posting: same row-building as Task 7 but for terms with `startBasis = 'Invoice Date'`; `startDate` = invoice date; **every row keyed on `salesInvoiceLineId`**, with `shipmentLineId` filled as lineage when the invoice line resolves to a shipment line (an unresolvable line still registers, keyed on the invoice line alone; tracked entities resolved through the shipment when available, else quantity row). Inside the posting transaction; failure rolls back.
