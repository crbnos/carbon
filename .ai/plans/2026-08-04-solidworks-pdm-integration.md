...
3. Methods (paths from the research file §7; response types as minimal TS interfaces — define only fields we consume):
   ...
   - `getThumbnail(fileId, version, folderId)` — `GET /api/{v}/files/{fileId}/{version}/thumbnails?folderId=`; the API replies with a redirect `Location`; follow it and return bytes + content type
...
**Steps:**
1. `files.ts` (GET loader): `requirePermissions` with the same scope as `integrations.onshape.documents.ts`; read `?q=` search text; `getSolidWorksPdmClient`; call `search` with a filename criteria built from `q`; hydrate results via `getFilesInfo`; return `{ files: [{ id, name, folderId, version }] }` filtered to extensions `.sldasm`/`.sldprt` (case-insensitive). Return plain objects (never `Response.json`). Empty `q` → return `{ files: [] }` without calling the API.
...
3. `SolidWorksPdmStatus` in `Icons.tsx`: clone `OnshapeStatus`, reading the workflow state name from the `solidworksPdmData` mapping metadata; a plain text badge is sufficient (no state-color mapping in v1).
...
**Steps:**
1. Pure functions: `extractCardValues(configInfos, { partNumberVariable, revisionVariable })` → picks the active configuration's values with `@`-tab fallback (input = the `ConfigInfo[]` shape from Task 2); `releaseMatchKey(partNumber, revision)` → delegates to the Onshape `releaseKey` convention; `colocatedCandidateNames(fileName)` → for `x.sldprt`/`x.sldasm` returns `["x.step", "x.stp"]`, for `x.slddrw` returns `["x.pdf"]`, case-insensitive, else `[]`; `classifyPdmFile(fileName)` → `"model" | "drawing" | "other"`.
2. Tests (vitest, mirror the Onshape test file's structure): active-config extraction with and without `@` fallback; empty part number → no match key; drawing shared-suffix matching via the reused `sharedNumberSuffix`; `colocatedCandidateNames` for all three extensions + uppercase variants; `classifyPdmFile` edge cases (`.SLDASM`, no extension). Also a `normalizeComputedBom` fixture test: hand-write a raw computed-BOM JSON fixture (documenting the assumed API shape per Task 6's escape hatch) and assert the flattened rows/levels/quantities.
...
5. Upsert the `"solidworks-pdm-release"` mapping row (metadata `{ fileId, version, fileName }`, `lastSyncedAt: now`, `companyId`, audit fields — copy column usage from wherever `onshape-sync-element.ts`/the sync route writes mappings).
...
2. Poll function: `{ id: "solidworks-pdm-release-poll", retries: 10, concurrency: { key: "event.data.companyId", limit: 1 } }`, event `carbon/solidworks-pdm-release-poll`. Body:
   - Load integration metadata; compute `since` = `mode === "backfill" ? null : lastReleaseSyncAt − 60min` (use `@internationalized/date`/ISO string math per the date-handling rule — no JS `Date` arithmetic beyond `new Date().toISOString()` stamps).
   - Candidate discovery, honoring `metadata.searchCapability`:
     - unset or `"state-search"`: call `client.search` with workflow-state-name + modified-since criteria. If the API rejects or ignores the state criterion (detectable: error response, or results whose bulk `getFilesInfo` current-state field ≠ released state), record `searchCapability: "folder-sweep"` in metadata and fall through.
     - `"folder-sweep"`: recursively `browseFolder` from the vault root, bulk `getFilesInfo` per folder batch, filter current state = `releasedStateName` (and modified ≥ `since` when incremental).
   - Fan out one `carbon/solidworks-pdm-file-sync` per candidate via `step.sendEvent` (batched).
   - On success write `metadata.lastReleaseSyncAt = sweepStartIso` (merge-update the metadata JSON — never replace it wholesale).
   
   Escape hatch: the search criteria body format is undocumented. Implement `state-search` behind a small request-builder function with the assumed format in one place; if the assumption can't be expressed from the research file, implement `folder-sweep` only, default `searchCapability` to `"folder-sweep"`, and report the gap — do not invent criteria syntax silently.
