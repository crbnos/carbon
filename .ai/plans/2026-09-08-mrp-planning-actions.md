...
## Progress
- [x] Task 1: Migration — `planningAction` table, enums, ladder columns, tolerance, RLS (96d53c0177; applied via crbn migrate, verified in DB)
- [x] Task 2: `pnpm run generate:types` (regenerated by crbn migrate; planningAction ×30 in types.ts; same commit)
- [x] Task 3: Shared pure reorder-sizing module — computePlanningOrders in @carbon/utils, 239 utils tests green, erp+utils typecheck clean
