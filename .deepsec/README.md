...
Currently configured project: `carbon` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/carbon/SETUP.md` to fill in
   `data/carbon/INFO.md`.
...
`--project-id carbon` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/carbon/`.
