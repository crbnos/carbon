# Agent setup for `carbon`

This is a deepsec scanning workspace. Project `carbon` was just registered
(target: `..`). Setup is incomplete — `data/carbon/INFO.md`
...
2. **Fill in `data/carbon/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id carbon
pnpm deepsec process --project-id carbon
```
