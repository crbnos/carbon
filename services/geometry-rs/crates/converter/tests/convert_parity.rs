...
//! bbox/volume/color within epsilon). Env GEOMETRY_FIXTURES points at a dir with
...
fn convert_matches_python() {
    let dir = match std::env::var("GEOMETRY_FIXTURES") {
        Ok(d) => d,
        Err(_) => {
            eprintln!("GEOMETRY_FIXTURES unset; skipping");
            return;
        }
    };
