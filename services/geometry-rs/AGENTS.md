...
1. **Planner shadow corpus** — `services/geometry/tests/capture_corpus.py` (env
   `GEOMETRY_CAPTURE_DIR`) captures the exact meshes + params + outcome from every
...
cd ../geometry && GEOMETRY_CAPTURE_DIR=/tmp/geom-corpus .venv/bin/python -m pytest tests/test_plan.py
...
GEOMETRY_CORPUS=/tmp/geom-corpus cargo test -p planner --test replay_greedy   # 5 cases
GEOMETRY_CORPUS=/tmp/geom-corpus cargo test -p planner --test replay_plan_parts  # 22 cases
GEOMETRY_CORPUS=/tmp/geom-corpus cargo test -p planner --test replay_fixed    # 8 cases
GEOMETRY_FIXTURES=/tmp/geom-fixtures cargo test -p converter --test convert_parity
GEOMETRY_FIXTURES=/tmp/geom-fixtures cargo test -p planner --test plan_step_smoke
...
```bash
GEOMETRY_DEV_MODE=true cargo run -p server   # listens on 0.0.0.0:8000 (GEOMETRY_BIND to override)
```

Env: `GEOMETRY_SERVICE_API_KEY` (bearer auth), `GEOMETRY_DEV_MODE=true` (allow
unauth + http + skip TLS verify, local only), `GEOMETRY_MAX_SOURCE_MB` (250),
`GEOMETRY_MAX_PARTS` (5000), `GEOMETRY_MAX_CONCURRENCY` (2),
`GEOMETRY_ALLOWED_URL_HOSTS`.
