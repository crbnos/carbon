...
| `./quality` | Inspection execution engine shared by ERP + MES (`upsertInspectionSample`, `upsertInspectionMeasurement`, `dispositionInspection` — optional one-shot `requireOpen`, `reconcileInspectionSamplingPlans`, `changeInspectionDocument`, `getOrCreateJobOperationInspection`, `createFirstArticleInspections` (AS9102 lots at release or one manual `only` request; idempotent per make method) + `loadFirstArticleNeedInput`, pure `valuateMeasurement` / `valuateGeometricMeasurement` (MMC/LMC bonus tolerance), re-exported from `supabase/functions/shared/inspection-verdict.ts`, which the dataset seed shares); Passed/Failed/Partial are all hard-terminal and samples linked from `productionQuantity.inspectionSampleId` are locked; every fn takes a `Kysely<KyselyDatabase>` first arg — authorize at the route, see `.claude/rules/inspection-system.md` |
| `./first-article` | Pure AS9102 need/due rules (`src/first-article.ts`, no I/O): `resolveFirstArticleNeeds` (the ONE decision shared by the release blocker, the generator and the job-header chip), `evaluateFirstArticleDue`, `formatFirstArticlePartDescription`, `firstArticleItemRevision`. See `.claude/rules/quality-certification-documents.md` |
