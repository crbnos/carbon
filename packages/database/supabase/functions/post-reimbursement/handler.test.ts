...
const companyId = "reimb-auth-company";
...
for (const type of ["post", "void"] as const) {
  Deno.test(`${type} refuses an API key without invoicing update`, async () => {
    await withAuthTransport({ invoicing_view: [companyId] }, async () => {
      let posts = 0;
      const response = await handlePostReimbursement(
        new Request("http://localhost/post-reimbursement", {
          method: "POST",
...
Deno.test("a malformed body never reaches the transaction", async () => {
  await withAuthTransport({ invoicing_update: [companyId] }, async () => {
    let posts = 0;
    const response = await handlePostReimbursement(
      new Request("http://localhost/post-reimbursement", {
        ...
        // No reimbursementId.
        body: JSON.stringify({ companyId, userId: "system" }),
      }),
      () => {
        posts++;
        return Promise.resolve({ journalId: "journal-1" });
    ...
    assertEquals(typeof body.message === "string", true);
    assertEquals(posts, 0);
  });
});
