...
Deno.test("null jobDueDate: every operation maps to null (nothing is due)", () => {
  const ops = [makeOp({ id: "op1" }), makeOp({ id: "op2" }), makeOp({ id: "op3" })];
  const result = needBy(ops, [["op2", "op1"], ["op3", "op2"]], null);
...
Deno.test("leaf anchor: serial 3-op chain due Friday gets Wed/Thu/Fri need-bys", () => {
  ...
  const result = needBy(ops, [["op2", "op1"], ["op3", "op2"]], "2026-01-16");
...
Deno.test("min-dependent constraint: a feeder with two consumers takes the earliest", () => {
  ...
  const result = needBy(ops, [["b", "a"], ["c", "a"]], "2026-01-16");
...
Deno.test("operationLeadTime on the consumer pulls feeders earlier, in working days", () => {
...
Deno.test("assemblyLeadTime applies only when the edge crosses make methods", () => {
  // Subassembly op feeding the parent method: pulled back 3 working days.
  const crossing = needBy(
    [
      makeOp({ id: "sub", order: 1, jobMakeMethodId: "mm-sub", assemblyLeadTime: 3 }),
  ...
  const sameMethod = needBy(
    [
      makeOp({ id: "sub", order: 1, jobMakeMethodId: "mm-root", assemblyLeadTime: 3 }),
...
Deno.test("With Previous: copies its partner's target dates; upstream chains off the copy", () => {
...
Deno.test("calendar day lengths: a 16h work center halves the day count of an 8h one", () => {
...
Deno.test("zero-hour days: a work center closed Fridays lands targets on Thursday", () => {
...
Deno.test("manual pin: stored dueDate passes through unchanged and propagates upstream", () => {
  ...
  const result = needBy(ops, [["b", "a"], ["c", "b"]], "2026-01-16");
...
Deno.test("calendarAdapters: hours per day from window minutes per weekday", () => {
  const windowsByWorkCenter = new Map([
    // Mon-Fri 08:00-16:00 => 8h working days
    ["wc8", expandCalendar(weekdayShifts("08:00", "16:00", [1, 2, 3, 4, 5]), RANGE_START, RANGE_END)],
    // Mon-Fri 06:00-22:00 => 16h working days
    ["wc16", expandCalendar(weekdayShifts("06:00", "22:00", [1, 2, 3, 4, 5]), RANGE_START, RANGE_END)],
...
Deno.test("calendarAdapters: workingDayTest covers open dates, weekends and closed days fail", () => {
  const windowsByWorkCenter = new Map([
    // Mon-Thu only: Fridays are zero-hour days for this center
    ["wc-nofri", expandCalendar(weekdayShifts("08:00", "16:00", [1, 2, 3, 4]), RANGE_START, RANGE_END)],
  ]);
  const locationWindows = expandCalendar(
    weekdayShifts("08:00", "16:00", [1, 2, 3, 4, 5]),
    RANGE_START,
...
Deno.test("integration: adapter-derived calendars drive the Friday-closed walk", () => {
  const windowsByWorkCenter = new Map([
    ["wc-nofri", expandCalendar(weekdayShifts("08:00", "16:00", [1, 2, 3, 4]), RANGE_START, RANGE_END)],
  ]);
  const locationWindows = expandCalendar(
  ...
  const adapters = calendarAdapters(windowsByWorkCenter, locationWindows, "UTC");
