-- =============================================================================
-- Fix: sales-order lines wrongly marked "Shipped" with NO shipment.
--
-- getSalesOrderJobStatus renders a green "Shipped" badge when the linked jobs are
-- complete AND salesOrderLine.sentComplete = true. sentComplete was being set to
-- true on JOB COMPLETION for every Non-Inventory item, via complete_job_to_inventory
-- (20260727031247) and recompute_service_line_fulfillment (20260723132843). Their
-- guard, `itemTrackingType = 'Non-Inventory'`, is too broad: it is a superset of
-- genuine Service items. A Non-Inventory *Part* (or Material/Tool/etc.) is
-- Non-Inventory-tracked but has salesOrderLineType <> 'Service', so the shipment
-- pipeline (the `create` edge fn skips ONLY salesOrderLineType='Service') DOES create
-- a shipment line for it and it ships normally — yet completion pre-marked it
-- fulfilled, producing the phantom "Shipped" badge with quantitySent set and
-- job.quantityShipped still 0.
--
...
-- -----------------------------------------------------------------------------
-- (1) complete_job_to_inventory — forked verbatim from 20260727031247 with ONLY
--     the SO-line fulfillment guard narrowed to genuine Service lines. All other
--     Non-Inventory branches (inventory/COGS skips) are unchanged and correct.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- (2) recompute_service_line_fulfillment — forked from 20260723132843, guard
--     narrowed the same way so reopening/cancelling a job only recomputes genuine
--     Service lines (a Non-Inventory Part's shipment-driven fulfillment is never
--     clobbered by job status changes).
-- -----------------------------------------------------------------------------
