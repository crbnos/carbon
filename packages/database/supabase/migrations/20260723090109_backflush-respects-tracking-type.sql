-- backflush_job_materials fork of 20260710044431 (verbatim) with ONE change:
-- the ledger/cost writes now respect the effective tracking type, mirroring the
-- issue edge function. Job-completion backflush previously wrote 'Consumption'
-- itemLedger rows (and consumed cost layers / posted WIP journals) for EVERY
-- non-serial/batch material, ignoring both the per-line
-- jobMaterial.itemTrackingType snapshot (20260722101327) and the item's own
-- Non-Inventory setting — so a line overridden to Non-Inventory was skipped by
-- the issue function but still hit the stock ledger when the job completed.
-- quantityIssued still updates for every line (same as the issue function).
...
BEGIN
  ...
  FOR v_material IN
    SELECT jm.id, jm."itemId", jm."quantityToIssue", jm."quantityIssued",
           ...
           COALESCE(jm."itemTrackingType", i."itemTrackingType") AS "effectiveTrackingType"
    FROM "jobMaterial" jm
    JOIN "item" i ON i."id" = jm."itemId" AND i."companyId" = jm."companyId"
  ...
  LOOP
    ...
    -- Mirror the issue edge function: a Non-Inventory line (per-line
    -- jobMaterial.itemTrackingType snapshot, falling back to the item) still
    -- records quantityIssued but writes NO stock ledger entry and never enters
    -- the cost arrays (no costLedger consumption, no WIP journal lines).
    IF v_material."effectiveTrackingType" = 'Inventory' THEN
