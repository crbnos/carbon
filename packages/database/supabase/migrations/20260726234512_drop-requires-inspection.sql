-- Drop the vestigial `requiresInspection` columns from `item`, `purchaseOrderLine`,
-- and `salesOrderLine`. Inbound (receipt) inspection is now gated entirely by the
-- item's Receipt-usage inspection-document assignment (`itemInspectionDocumentAssignment`),
-- not a per-item boolean. See `.claude/rules/inspection-system.md`.
--
-- The `item` column is referenced by the item-detail RPCs and by the
-- `parts`/`tools`/`materials`/`consumables`/`services` views; the line columns are
-- referenced (transitively, via `pl.*` / `sl.*`) by `purchaseOrderLines` /
-- `salesOrderLines`. Views hard-block a column drop, so we drop them first, drop the
-- columns, then recreate the views (and the RPCs) without the column.
--
-- Idempotent: guarded with IF EXISTS / IF NOT EXISTS so a partial re-run over
-- committed state re-converges. No downstream object depends on these views, so no
-- CASCADE is needed; grants are re-acquired via Supabase default privileges (same
-- pattern as `20260711011724_exclude-draft-journals.sql`).

-- 1. Drop the views that reference the columns.
...
-- 2. Drop the item-detail RPCs (recreated below without the column; the RETURNS
--    TABLE row type changes, so DROP + recreate rather than CREATE OR REPLACE).
...
-- 3. Drop the columns.
ALTER TABLE "item" DROP COLUMN IF EXISTS "requiresInspection";
ALTER TABLE "purchaseOrderLine" DROP COLUMN IF EXISTS "requiresInspection";
ALTER TABLE "salesOrderLine" DROP COLUMN IF EXISTS "requiresInspection";

-- 4. Recreate the views (verbatim from the newest prior definitions, with every
--    `requiresInspection` reference removed). The `*`-expansion views (`parts`,
--    `purchaseOrderLines`, `salesOrderLines`) are recreated as-is — the dropped
--    column simply no longer expands.
...
CREATE VIEW "services" WITH (security_invoker = true) AS
 WITH latest_items AS (
         SELECT DISTINCT ON (i."readableId", i."companyId") i.id,
            i."readableId",
            i.name,
...
-- 5. Recreate the item-detail RPCs without the `requiresInspection` column
--    (removed from both RETURNS TABLE and the SELECT list).
