-- Unify inspection dispositions with production outcomes.
--
-- 1) Provenance/idempotency links: a productionQuantity row can record which
--    inspection (and which sample) drove it. The partial UNIQUE index on
...
CREATE INDEX IF NOT EXISTS "productionQuantity_inspectionId_idx"
  ON "productionQuantity" ("inspectionId");
CREATE UNIQUE INDEX IF NOT EXISTS "productionQuantity_inspectionSampleId_key"
  ON "productionQuantity" ("inspectionSampleId")
  WHERE "inspectionSampleId" IS NOT NULL;

-- 2) sync_finish_job_operation (fork of 20260706181125): the zero-completions
--    fallback to the full job quantity now applies ONLY when the job recorded
--    no scrap/rework — a fully-scrapped job closes without receiving stock.
...
-- 3) complete_job_to_inventory (fork of 20260714043017): the serial branch
--    excludes 'Rejected' entities (scrapped at inspection) from both the
--    Assembly Output ledger loop and the Available release.
