-- Picked-material return timing (spec: .ai/specs/2026-08-04-picked-material-return-timing.md)
--
-- 1. companySettings.returnPickedMaterialTiming: when unconsumed picked material
--    flushes back from the work-center lineside bin to the warehouse — at job
--    completion (default, current behavior) or at each operation's completion.
-- 2. pickingListLine.quantityReturned: returns book here instead of decrementing
--    quantityPicked. The update_picking_list_status trigger reacts to
--    quantityPicked/status changes and demotes Completed/Partial headers back to
--    In Progress — correct for an operator unpick (work regression), wrong for a
--    post-completion return sweep. quantityPicked stays gross-picked; net staged
--    at lineside = quantityPicked - quantityReturned.
-- 3. backflush_job_materials: verbatim fork of the 20260713190909 definition,
--    adding a lineside on-hand clamp (untracked stock, bins with a workCenterId
--    only) so a completion-time top-up can never drive a lineside bin negative
--    after an early per-operation return.
