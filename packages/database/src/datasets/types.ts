...
export type AssemblyStepSpec = {
  ...
  /** Optional explode motion for the assembler player. Unit direction + mm. */
  motion?: {
    type: string;
...
export type AssemblySpec = {
  ...
  /**
   * When true, pin this instruction on the item's first method operation so
   * copyMethodToJob snapshots the 3D steps onto the job. Leave false when the
   * operation already has a procedure checklist (the four original demos).
   */
  attachToMethod?: boolean;
  /** geometryHash → item readableId for assemblyComponentMapping. */
  nodeItemMap?: Record<string, string | null>;
};
...
export type ProductionData = {
  ...
  /** Extra bundled assemblies (same industry assets dir). */
