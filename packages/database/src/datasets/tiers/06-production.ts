...
async function seedAssembly(ctx: Ctx, spec: AssemblySpec): Promise<void> {
  ...
  if (spec.nodeItemMap) {
    for (const [geometryHash, readableId] of Object.entries(spec.nodeItemMap)) {
      const mappedId = readableId ? ctx.refs.items[readableId]?.id : undefined;
  ...
  if (spec.attachToMethod && spec.item) {
    const makeMethodId = ctx.refs.items[spec.item]?.makeMethodId;
    if (makeMethodId) {
      await ctx.client.query(
        `UPDATE "methodOperation"
         SET "assemblyInstructionId" = $1, "operationType" = 'Assembly'
         WHERE "makeMethodId" = $2 AND "companyId" = $3 AND "order" = 1`,
        [instructionId, makeMethodId, ctx.companyId]
      );
    }
  }
}
...
export async function runTier6(ctx: Ctx): Promise<void> {
  ...
  for (const spec of assemblies) {
    ctx.log(`assembly ${spec.name}`);
    await seedAssembly(ctx, spec);
  }
