...
- **Aux spawners** (`services/apps.ts`): `spawnAssembler` (cargo) and `spawnEmailPreview` (`@carbon/documents` `email:gallery --serve` on `PORT_EMAIL` — every email template rendered on one page, refresh to re-render) — opt-in picker apps with their own spawners, not react-router dev servers
