...
- **App selection** (`app-selection.ts`): `resolveAppSelection()` folds `--app` / `--all` / `--no-apps` into one `AppSelection` (`none` | `all` | `explicit` | `prompt`) in `main.ts`, BEFORE `up()` touches docker, and throws on a contradictory pair instead of letting one flag silently win. `--app` takes ids repeated or comma-separated and rejects unknown ones (unlike the `CARBON_DEV_APPS` env path in `pickApps`, which drops them). Only `prompt` can block on input — add new app-selecting flags here, not in `up.ts`
