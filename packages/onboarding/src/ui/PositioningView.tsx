  ...
  return (
    <div className="w-full max-w-3xl mx-auto flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <VsColumn
          title="What others do better"
          tone="red"
          points={OTHER_STRENGTHS}
        />
        <VsColumn
          title="What Carbon does better"
          tone="blue"
  ...
  return (
    <section className="rounded-2xl border bg-card shadow-button-base p-5">
      ...
      <div className="flex flex-col gap-4">
        {points.map((p) => (
          <div key={p.lead}>
            <div className="text-sm font-medium">{p.lead}</div>
            <div className="text-sm text-muted-foreground mt-0.5">{p.body}</div>
          </div>
        ))}
      </div>
    </section>
