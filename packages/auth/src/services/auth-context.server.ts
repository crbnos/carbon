...
export class AuthClientScope {
  ...
  // the factory (not a built client) so construction stays lazy.
  //
  // Last-writer-wins: a later setFactory replaces both the factory AND the
  // memoized `built` client. RR-7 v8_middleware shares one ALS cell across a
  // request's parallel loaders, so without this, a parent layout that
  // pre-sets an RLS factory would silently downgrade a child loader that
  // calls `requirePermissions(..., { bypassRls: true })` — the child's
  // setFactory would be ignored because the parent already triggered a
  // build. Already-issued client references (held by in-flight service
  // calls) are unaffected; only subsequent `getAuthClient()` calls see the
  // new factory. This matches main, where `client` was a value bound at
  // call time, not a shared mutable cell.
  static setFactory(factory: () => unknown): void {
    ...
    cell.built = undefined;
  }
}

// Establish a request-scoped client cell with a fixed, pre-built client.
// Used by routes that need the service-layer `getAuthClient()` accessor to
// work but do NOT yet have an identity (e.g. they must read a discriminator
// record first to learn the target companyId). Pair with a subsequent
// `runWithSystemContext` once the companyId is known, or fold the whole
// loader into `runWithSystemContext` directly.
//
// `client` is captured by value rather than factory because the caller has
// already constructed it; the lazy-factory indirection in `setFactory` exists
// for `requirePermissions`' decision logic, which is not relevant here.
export function runWithSystemClient<T>(client: unknown, fn: () => T): T {
  return AuthClientScope.run(() => {
    AuthClientScope.setFactory(() => client);
    return fn();
  });
}

// Establish a complete request context (identity + client) for routes that
// act on behalf of the SYSTEM rather than a logged-in user — public webhooks
// (companyId is the URL target, not an actor's company), public share pages,
// background/system endpoints. These routes have no session, so the auth
// middleware leaves both ALS cells empty; service code that reads identity
// (`AuthContextHolder.get().companyId`) would otherwise throw fail-closed.
//
// Why combined (not two separate calls): the client and identity scopes are
// invariants that must be opened TOGETHER. Setting only `AuthClientScope`
// (the prior pattern) lets a service call build a client successfully and
// then explode on the next line when it reads identity. One function call
// makes the invariant un-skippable.
//
// `client` is the resolved Supabase client to register (typically the service-
// role client, since these routes have no RLS session). It is captured by
// value, not factory — the caller has already decided; lazy construction is
// unnecessary here and keeps the API symmetric with what the caller actually
// has (a client, not a builder).
//
// `userId`/`companyGroupId` default to `""`: there is no user actor for a
// webhook/share request, and `companyGroupId` is derivable from `companyId`
// only via a DB lookup the caller has not yet done. Services that need them
// must accept them as explicit parameters (see `getIntegration(id, companyId?)`
// for the established pattern of "optional override, ALS fallback").
export function runWithSystemContext<T>(
  ...
  return runWithSystemClient(client, () =>
    AuthContextHolder.run(
      {
        client: undefined,
        companyId: identity.companyId,
        userId: identity.userId ?? "",
        sessionUserId: identity.sessionUserId ?? identity.userId ?? "",
