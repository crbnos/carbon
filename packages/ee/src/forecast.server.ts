 ...
 * company's `demandForecast` rows, gated to the Business plan via the
 * `FORECAST` feature. The MRP consumer (`planning/mrp/mrp.ts`) that READS
 * forecasts is a separate concern and is not gated here.
 *
 ...
 * ungated `production_upsertDemandForecasts` / `production_deleteDemandForecasts`
 * MCP tools — an ungated MCP write path would otherwise route around the gate.
 *
