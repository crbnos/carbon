...
import RuleViolationModal from "./violation-modal";

/**
 * Shape every rule-aware server action must return when it wants the
 ...
 */
export type RuleViolationPayload = {
...
type UseRuleViolationsResult<T> = {
...
/**
 * Centralised handler for rule violations + action errors.
 *
 * Wraps a `useFetcher` and:
 * - toasts any `error.message` returned by the action
 * - opens `<RuleViolationModal>` when the action returns `violations`
 ...
 */
export function useRuleViolations<T = unknown>({
  action,
  onSuccess
}: UseRuleViolationsOptions): UseRuleViolationsResult<T> {
  ...
  const [staged, setStaged] = useState<RuleViolationPayload | undefined>(
    undefined
  );
  ...
  const data = fetcher.data as RuleViolationPayload | undefined;
