...
export const SECRET_KEYS: Record<string, string[]> = {
  ...
  // webhookSigningSecret is a shared HMAC key — possession lets anyone forge a
  // delivery the receiver accepts — so it belongs in the vault rather than
  // plaintext in the metadata column, where it would also reach the browser via
  // the settings drawer loader and sit in the Redis integrations cache.
  onshape: [
