...
Enterprise edition — integrations registry, accounting sync (Xero, QuickBooks Online, Rillet), plan gating, exchange rates, Slack, email, Jira, Linear, Onshape, the shared rules layer (storage + sales rules), the planning engines (MRP + finite scheduling), and SAML SSO.
...
- **Exports**: `./accounting`, `./planning`, `./plan`, `./plan.server`, `./rules` (client-safe storage + sales rule services/UI), `./rules.server` (both evaluators), `./exchange-rates.server`, `./slack.server`, `./hooks.server`, `./sso.server`, `./jira`, `./linear`, `./rillet/hooks.server`, `./xero/hooks.server`, etc.
