...
Enterprise edition — integrations registry, accounting sync (Xero, QuickBooks Online, Rillet), plan gating, exchange rates, Slack, email, Jira, Linear, Onshape, and the shared rules layer (storage + sales rules).
...
- **Exports**: `./accounting`, `./plan`, `./plan.server`, `./rules` (client-safe storage + sales rule services/UI), `./rules.server` (both evaluators), `./exchange-rates.server`, `./slack.server`, `./hooks.server`, `./jira`, `./linear`, `./rillet/hooks.server`, `./xero/hooks.server`, etc.
