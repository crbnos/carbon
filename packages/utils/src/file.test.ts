...
describe("getThumbnailPath and isThumbnailPath", () => {
  it("derives the paired thumbnail path preserving extension", () => {
    expect(getThumbnailPath("co/parts/abc.png")).toBe("co/parts/abc.thumb.png");
    expect(getThumbnailPath("co/job/op/step/123/photo.jpg")).toBe(
      "co/job/op/step/123/photo.thumb.jpg"
    );
    expect(getThumbnailPath("file-without-ext")).toBe("file-without-ext.thumb");
  });

  it("identifies whether a path is a thumbnail", () => {
    expect(isThumbnailPath("co/parts/abc.thumb.png")).toBe(true);
    expect(isThumbnailPath("co/parts/abc.png")).toBe(false);
  });
});
