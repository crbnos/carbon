...
| `accounting-posting` | Original journal role vocabulary: `classifyAccountingPostingRole` maps a journal line's description by EXACT match — `Accounts Receivable`/`IC Receivables` → Receivables, `Accounts Payable`/`IC Payables` → Payables, `Shipping Revenue`, `Sales Account` — and returns `null` for anything else, which is how a `VOID: …` reversal line falls out (there is no explicit void branch). Also the customer-deposit journal descriptions `CUSTOMER_DEPOSIT_DESCRIPTION` ("Customer Deposit") and `CUSTOMER_DEPOSIT_APPLIED_DESCRIPTION` ("Customer Deposit (applied)"), used by `post-payment` (its journal builder and its prior-credit source lookup) |
...
| `revenue-schedule` | ISO-date arithmetic on `YYYY-MM-DD` strings (`parseIsoDate`, `formatIsoDate`, `daysInMonth`, `monthEnd`, `daysBetweenInclusive`, `addDays`) and `spreadStraightLine` (a revenue schedule spread over days with cumulative rounding) — used by `post-sales-invoice` deferrals and the rental/lease modules |
| `rental-billing` | Rental billing period math: `bestRateCharge`, `fixedRateCharge`, `calendarMonthCharge`, `billingHorizon`, `generateRentalBillingPeriods` (create / recut / early-return adjustments against persisted periods). Shared by `post-rental-agreement`, `@carbon/database/rental-billing` and the ERP |
| `lessor-lease` | ASC 842 lessor math: `presentValue`, `classifyLessorLease`, `buildLessorSchedule`, `wholeMonthsInTerm`, `leasePaymentTerms`, `certainPurchaseOption`, `classifyRentalLine`, `salesTypeRequirementError` (end date, fair value, whole billing periods). One function for the ERP's Draft preview and what activation stores |
