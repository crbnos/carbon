...
<Screenshot src="/screens/reference-parts-csv.png" label="Parts list with the Download CSV icon in the table header" caption="Every Carbon data table exports its current view. Filter and arrange columns first, since the export mirrors exactly what's on screen." />
...
<FieldTable>
  ...
  <Field name="Inventory, Batch, and Serial Quantities">From **Inventory → Quantities**. Adds opening stock for items that already exist, one import per <Term id="item-tracking-type">tracking type</Term>. See [Opening stock](#opening-stock).</Field>
</FieldTable>
...
<FieldTable>
  <Field name="Inventory Quantities">For items tracked as **Inventory**. Columns: Part Number, Revision, Location, Storage Unit, Quantity, Comment.</Field>
  <Field name="Batch Quantities">For items tracked as **Batch**. Columns: Part Number, Revision, Location, Storage Unit, Batch Number, Quantity, Expiration Date, Comment. Each row creates a new batch.</Field>
  <Field name="Serial Quantities">For items tracked as **Serial**. Columns: Part Number, Revision, Location, Storage Unit, Serial Number, Expiration Date, Comment. There's no Quantity column: every row is one unit.</Field>
</FieldTable>
...
<Callout type="warn" title="Re-importing an Inventory Quantities file adds the stock again">
...
<Cards>
  ...
  <Card title="Inventory" href="/docs/reference/inventory">On-hand quantities and the manual adjustments the opening-stock imports post.</Card>
</Cards>
