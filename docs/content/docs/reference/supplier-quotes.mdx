...
description: "A supplier's priced response to your RFQ, one per supplier, ready to convert into a purchase order."
---

A **supplier quote** is a vendor's priced answer to a <Term id="rfq">purchasing RFQ</Term>: for the items you asked about, at the quantities you named, here's my price, my lead time, and my shipping. It's the buy side of a quote, and it is not a <Term id="quote">sales quote</Term>. A sales quote is what *you* send a customer; a supplier quote is what a *supplier* sends *you*.

You keep one quote per supplier per exchange, and once you've compared them you convert the winner into a <Term id="purchase-order">purchase order</Term>. Converting also writes the supplier's prices back onto the item's supplier part, so the next order already knows what this vendor charges.
...
<Callout type="note" title="One quote, one supplier">
...
<FieldTable>
  <Field name="Item" type="reference">The part, material, tool, consumable, or service being priced. A `G/L Account` line prices non-stock spend instead.</Field>
  <Field name="Quantity" type="number[]">The quantities you're asking the supplier to price. Each one gets its own price row.</Field>
  <Field name="Unit price" type="number">The supplier's price at that quantity, in the supplier's currency. `unitPrice` in your base currency is derived from the exchange rate.</Field>
  <Field name="Lead time" type="number">Days to deliver at that quantity.</Field>
  <Field name="Shipping cost" type="number">Per-unit shipping at that quantity, in the supplier's currency.</Field>
  <Field name="Conversion factor" type="number">Inventory units per one purchase unit, when the supplier sells in a different unit than you stock in.</Field>
</FieldTable>

<Callout type="note" title="Prices are quoted in the supplier's currency">
...
<StatusFlow>
  <Status name="Draft">Being built. The only editable state: you can add lines, enter prices, and adjust dates here.</Status>
  <Status name="Active" accent>Finalized. Locked, and the state the comparison and convert-to-order flows read.</Status>
  <Status name="Expired" terminal>Past its expiration date. Pricing is no longer trustworthy.</Status>
  <Status name="Declined" terminal>The supplier declined to quote.</Status>
  <Status name="Cancelled" terminal>Withdrawn before it went anywhere.</Status>
</StatusFlow>

<Callout type="warning" title="Locked once it leaves Draft">
...
<Callout type="note" title="Converting updates item costs">
...
For the full narrative, from sending the RFQ through comparing quotes to raising the order, see the [RFQ to PO guide](/guides/rfq-to-po). For what happens to the order once it exists, see [purchase orders](/docs/reference/purchase-orders).
