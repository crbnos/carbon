...
<FieldTable>
  ...
  <Field name="sourcePaymentId" type="payment">A *prior* posted payment supplying on-account credit. `NULL` means the row is funded by the applying payment's own cash. Either way `paymentId` stays the applying and voiding owner, so a credit-funded row can only sit on a cash payment, never on a memo.</Field>
  ...
  <Field name="sourceAmount">Principal consumed **in the funding source's document currency**, stored independently of `appliedAmount`.</Field>
  <Field name="appliedAmount">Target-document principal relieved, **in company base currency**.</Field>
  <Field name="discountAmount">An early-payment discount you're granting, in **base currency**. **Cash payments only** — a memo-sourced settlement can't carry one.</Field>
  <Field name="writeOffAmount">A remainder you're forgiving to the write-off account, in **base currency**. **Cash payments only.**</Field>
  <Field name="fxGainLossAmount">The realized FX this one application books, in **base currency**: positive is a gain, negative a loss. Server-calculated at posting, never entered.</Field>
...
A customer receipt can also be a <Term id="customer-deposit">customer deposit</Term>. On a **"Payment from Customer"**, pick one of that customer's open sales orders or Draft or Active [rental agreements](/docs/reference/rental-agreements#deposits) under **"Deposit for"**, and its unapplied cash posts to **Customer Prepayments** instead of on-account credit. It is applied to an invoice like any other credit.
...
<Screenshot label="Payment from Customer form with the Deposit for picker open, listing the customer's open sales orders and rental agreements" caption="Deposit for narrows to the selected customer's open documents." />
