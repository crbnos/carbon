...
A customer receipt can also be a <Term id="customer-deposit">customer deposit</Term>: pick the sales order or [rental agreement](/docs/reference/rental-agreements#deposits) under **"Deposit for"**, and its unapplied cash posts to **Customer Prepayments** instead of on-account credit. It is applied to an invoice like any other credit, and a disbursement with **"Refund deposit for"** pays the balance back.
