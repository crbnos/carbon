---
title: Documents
description: A shared file library with favorites, labels, search, and AI extraction that reads PDFs and fills a form for you.
---
...
<FieldTable>
  <Field name="All Documents">Every active file in the company you have read access to.</Field>
  <Field name="My Documents">Files you uploaded (`createdBy` = you).</Field>
  <Field name="Recent">Sorted by last activity — the newest view, download, label, or edit first.</Field>
  <Field name="Pinned">Only files you have pinned.</Field>
  <Field name="Trash">Soft-deleted files, waiting to be restored or purged.</Field>
</FieldTable>
...
<Callout type="info" title="Trash is a soft delete, not a purge">
...
<Callout type="note" title="Groups are Carbon's company groups">
Read and write groups are drawn from your <Term>company group</Term> membership, the same groups used elsewhere for scoping. Set them when you edit a document's properties; a file with no matching read group for a user is simply invisible to that user.
</Callout>
...
<FieldTable>
  <Field name="Sales side">Quote, Request for Quote, Sales Order, Sales Invoice, Shipment.</Field>
  <Field name="Purchasing side">Purchase Order, Purchase Invoice, Purchasing Request for Quote, Supplier Quote.</Field>
  <Field name="Production & quality">Job, Gauge Calibration Record, Issue.</Field>
  <Field name="Items">Part, Material, Tool, Consumable, Service.</Field>
</FieldTable>

The attachment is stored as a `sourceDocument` (the type) plus a `sourceDocumentId` (the record's id) on the file (`documents.service.ts:154-183`). Because the file also lives in the shared explorer, you can find it either by opening the record or by searching the library. See the [sales orders](/docs/reference/sales-orders), [purchase orders](/docs/reference/purchase-orders), and [jobs](/docs/reference/jobs) references for where each record surfaces its files.
...
<Callout type="info" title="It resolves ids for you, it doesn't invent them">
...
<Callout type="warning" title="Extraction is a draft, not a commit">
...
<Callout type="note" title="Not the document template customizer">
