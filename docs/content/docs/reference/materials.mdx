...
<FieldTable>
  <Field name="Substance" type="materialSubstance">What the material is made of: steel, aluminum, stainless, titanium. Has a `name` and a `code`.</Field>
  <Field name="Form" type="materialForm">Its shape or product form: sheet, plate, round bar, ingot. Has a `name` and a `code`.</Field>
  <Field name="Dimension" type="materialDimension">A named size, scoped to a **form** (a sheet's dimension list differs from a bar's). Carries `isMetric`.</Field>
  <Field name="Finish" type="materialFinish">A surface finish, scoped to a **substance**.</Field>
  <Field name="Grade" type="materialGrade">A material grade, scoped to a **substance** (for example a steel grade like 1018).</Field>
  <Field name="Type" type="materialType">A named product type keyed to a **substance and a form** together, such as hot-rolled steel plate. Has a `name` and a `code`.</Field>
</FieldTable>

<Callout type="note" title="The lookups cascade">
...
<FieldTable>
  <Field name="materialSubstance" type="(code, companyId)">Substances are unique by code within a company.</Field>
  <Field name="materialForm" type="(code, companyId)">Forms are unique by code within a company.</Field>
  <Field name="materialDimension" type="(materialFormId, name, companyId)">A dimension name is unique within its form.</Field>
  <Field name="materialFinish" type="(materialSubstanceId, name, companyId)">A finish name is unique within its substance.</Field>
  <Field name="materialGrade" type="(materialSubstanceId, name, companyId)">A grade name is unique within its substance.</Field>
  <Field name="materialType" type="(substance, form, code) + (substance, form, name)">A type is unique by both code and name within a substance-and-form pair.</Field>
</FieldTable>

<Callout type="info" title="Substance and form carry more; the rest are lean">
...
Because the join is on the readable id, one material can span several item records: separate **revisions**, and a separate item per stock size. The `sizes` you assign become selectable variants on [jobs](/docs/reference/jobs) and purchase orders, each its own item row sharing the material's id.

<Callout type="note" title="Only substance and form are required to generate a material">
...
<Cards>
  <Card title="Items" href="/docs/reference/items">
    A material is an item of type *Material*; item fields like tracking and unit of measure live here.
  </Card>
  <Card title="Methods & sourcing" href="/docs/reference/methods">
    Materials become the components of a method's bill of materials.
  </Card>
</Cards>
