...
<FieldTable>
  ...
  <Field name="includeMaterialsOnTraveler" type="boolean">When enabled, job traveler PDFs include a materials section showing item number, description, method type, and quantity per job method. Off by default.</Field>
  <Field name="autoSelectMaterialWithoutPickingList" type="boolean">When enabled, the MES issue-material screen auto-fills the FEFO lot suggestion for materials not on a picking list. The single-parent genealogy guard still applies.</Field>
  <Field name="incompletePickingListPolicy" type="enum">How short picks are handled: `warn` (default) shows an acknowledge modal; `error` blocks completion until the shortage is resolved. A **Partial** status marks acknowledged short picks as a locked terminal state.</Field>
</FieldTable>
