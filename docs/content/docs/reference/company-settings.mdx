...
<FieldTable>
  ...
  <Field name="includeMaterialsOnTraveler" type="boolean">When on, the job traveler PDF includes a materials (bill-of-materials) section. Edited under **Settings → Production**.</Field>
  <Field name="autoSelectMaterialWithoutPickingList" type="boolean">Auto-fills the FEFO lot suggestion when issuing material without a picking list on the MES floor. Edited under **Settings → Production**.</Field>
  <Field name="incompletePickingListPolicy" type="enum">How to handle short picks: `warn` (acknowledge modal) or `error` (block). Edited under **Settings → Inventory**.</Field>
</FieldTable>
