...
<Callout type="note" title="Posted counts are terminal — no rectification workflow.">
Prior to August 2026, inventory counts could be "rectified" after posting (Posted → Draft → re-post). That workflow no longer exists. A posted count stays posted; any fix is made as a correction **to the movement**, not by reopening the count. This approach keeps the audit trail clear and treats corrections as first-class ledger entries.
</Callout>
