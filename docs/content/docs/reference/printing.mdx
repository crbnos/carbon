...
description: "Printer routing, ZPL and PDF labels, and the print-job queue that turns a receipt or a finished job into a label on a shelf."
---
...
<FieldTable>
<Field name="productLabel" type="zpl default">Labels for <Term id="tracked-entity">tracked entities</Term> (serial or batch numbers). Printed from a `Receipt`, `Shipment`, `Operation`, `Entity`, `Job`, `Split`, or `StockTransfer`.</Field>
<Field name="storageUnitLabel" type="zpl default">Labels for shelves, bins, and storage locations. Printed from a `StorageUnit`.</Field>
<Field name="kanbanCard" type="pdf only">Replenishment cards for <Term>kanban</Term> bins. Printed from a `Kanban`.</Field>
</FieldTable>
...
<FieldTable>
<Field name="name" type="text" required>What you call the printer, unique per location. "e.g. Zebra 2x1".</Field>
<Field name="format" type='"zpl" | "pdf"' required>"ZPL (Thermal Label)" or "PDF (Document)". A CHECK constraint enforces the two values.</Field>
<Field name="mediaSizeId" type="text" required>The label size this printer loads. ZPL printers only offer thermal (ZPL-capable) sizes; PDF printers offer all.</Field>
<Field name="printerUrl" type="url" required>The ProxyBox HTTP endpoint jobs are delivered to, e.g. `https://pbx-XXXX.pbxz.cloud/api/v1/print/…`.</Field>
<Field name="apiKey" type="text">Optional key sent as an `X-API-Key` header on delivery.</Field>
<Field name="templateId" type="text">A BinderyPress template id. "Leave blank for built-in" — leave empty to use Carbon's built-in renderers.</Field>
<Field name="locationId" type="text">The location the printer lives at (optional; a route can be company-wide).</Field>
</FieldTable>
...
<FieldTable>
<Field name="default">The location's fallback printer. Every other context inherits this one when it has no printer of its own.</Field>
<Field name="shipping">Used when the source document is a `Shipment`.</Field>
<Field name="receiving">Used when the source document is a `Receipt`.</Field>
<Field name="inventory">Used for a `StockTransfer` or a `StorageUnit`.</Field>
<Field name="workCenter">Per work center, for labels printed from a job operation. A work center with no assignment inherits the location default outright.</Field>
</FieldTable>
...
<StatusFlow>
<Status name="generating">The job exists; its content is being rendered (ZPL text or a base64 PDF).</Status>
<Status name="queued">Rendered and ready. Carbon sends a delivery event to push it to the printer.</Status>
<Status name="printing">Handed to ProxyBox; delivery is in flight.</Status>
<Status name="completed" accent>Delivered, or rendered with no printer to send to (a download-only job).</Status>
<Status name="failed" terminal>Rendering or delivery failed. The `error` column holds the reason and `attempts` counts tries.</Status>
</StatusFlow>
...
Two places set a size. A printer route pins its own `mediaSizeId` (the size loaded in that physical printer). And the company has a **default product-label size** (`companySettings.productLabelSize`), set on the document-template screen, used by the interactive label-download routes when no route dictates one. See [Company settings](/docs/reference/company-settings) for where that default lives, and [Documents](/docs/reference/documents) for the customizer that styles the built-in label templates.

<Cards>
  <Card title="Company settings" href="/docs/reference/company-settings">
    Where the default product-label size and other company-wide printing defaults live.
  </Card>
  <Card title="Inventory & locations" href="/docs/reference/inventory">
    Storage-unit labels and the tracked entities most print jobs describe.
  </Card>
  <Card title="Traceability" href="/docs/reference/traceability">
    The serial and batch numbers a product label carries.
  </Card>
  <Card title="Kanban" href="/docs/reference/kanban">
    Replenishment bins and the PDF cards you print for them.
  </Card>
</Cards>
