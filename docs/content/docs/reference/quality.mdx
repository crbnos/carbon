...
Whatever raised it, an inspection is worked on one screen: a features × samples grid laid over the item's ballooned drawing. Each column is a sample (scanned serials and batches label their own column); each row is a feature to measure, recording a value or a pass/fail. Sample and lot status are derived from those readings against tolerance. The same screen serves an inbound lot held off a receipt and an in-house **Inspection** <Term id="job-operation">job operation</Term> — an Inspection operation resolves the part's inspection document and runs this grid on the shop floor.

## Also in quality

The module also covers [**gauges and calibration**](/docs/reference/calibration) (with calibration-due tracking), **controlled documents** (versioned procedures and work instructions, with one active version at a time), **inspection documents** (ballooned FAI drawings and their features), and a **risk register**.
