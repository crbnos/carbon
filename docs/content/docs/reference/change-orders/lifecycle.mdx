...
description: The five change-notice stages, the Implementation lock and Reopen, the impact panel, the diff review, and exactly what release does when a change notice reaches Done.
---
...
<StatusFlow>
  <Status name="Draft">The change notice is being written. Pick affected items, set change types, and edit each draft method. Nothing is broadcast.</Status>
  ...
  <Status name="Implementation" accent>The release stage. The engineering content freezes here, and the **"Release"** action becomes available; it reviews every affected item's diff and applies the change. Advancing here broadcasts.</Status>
  <Status name="Done" terminal>Released and locked. The change notice becomes a read-only historical record; broadcasts on entry.</Status>
</StatusFlow>
...
- **Supplier Parts** — for a purchased Revision or New Part, the [suppliers you set up on the
