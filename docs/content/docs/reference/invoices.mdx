...
<Screenshot label="Purchase invoice line form with the Item, GL Account and Asset tabs, the Asset tab selected" caption="A purchase invoice line picks its type by tab: an item, a ledger account, or a fixed asset." />

A **Rental** line is never added by hand: [rental agreement](/docs/reference/rental-agreements) billing writes one per billing period or charge, with no item, and it posts to lease accounts rather than to sales.
