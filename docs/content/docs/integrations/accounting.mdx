...
A **Run Initial Sync** action kicks off the first backfill on demand. After that, changes flow continuously,
matched to the same Xero record each time so a Carbon customer or invoice never duplicates.

<Callout type="warn">
Xero only appears when its OAuth client is configured server-side (`XERO_CLIENT_ID`). Without it the
integration is hidden — see [Environment variables](/docs/platform/environment-variables).
</Callout>
