...
<Field name="Email Domains" type="text">A comma-separated list of domains, e.g. `example.com, example.org`. Domains are lowercased and must be bare hostnames — no `@`, no spaces. A domain can belong to **one company only**: registering a domain another company already claimed fails with **"Domain example.com is already registered to another company"**.</Field>
</FieldTable>
