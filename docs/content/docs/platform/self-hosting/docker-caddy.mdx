...
Everything for this recipe lives in [`contrib/deployment/simple-docker-caddy`](https://github.com/crbnos/carbon/tree/main/contrib/deployment/simple-docker-caddy). The `deploy.sh` script wraps the whole lifecycle.
...
<Steps>
  ...
  ```bash
  cd /opt/carbon/contrib/deployment/simple-docker-caddy
...
See [Environment variables](/docs/platform/self-hosting/environment-variables) for the full configuration surface, or the managed [AWS deployment](/docs/platform/deployment) for the cloud path.
