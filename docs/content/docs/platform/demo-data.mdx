...
The stock movements and payment settlements behind those statuses are filled in too, so on-hand quantities and invoice balances agree. The templates add no uploaded files, so [Documents](/docs/reference/documents) starts empty.
