...
<Diagram name="map" caption="Everything Carbon runs. The loop on the right is the surprise: Inngest is a timer and a queue, but the job itself executes back inside the ERP." />
...
<Diagram name="click" caption="One click, all the way down. Every hop crosses a layer, and everything above the dashed line happens while the user waits." />
...
<Diagram name="doors" caption="Four ways into the database. Only the first one leaves the database's own security rules switched on." />
...
<Diagram name="paths" caption="Writes land in tables; reads come back through views, which is where computed values like order totals live." />
...
<Diagram name="events" caption="A subscribed write becomes up to six side effects, none of them visible from the code that did the write." />
...
<Diagram name="runs" caption="The same code base in three shapes, and the four CI workflows that move it between them." />
...
<Diagram name="triage" caption="Start from what the user saw, then read down the one column that matches." />
