...
- Purchase to Order — you purchase it. The motors come from a supplier, so they flow through [purchasing](/guides/rfq-to-po) as a purchase order, never touching the floor.
- Pull from Inventory — you pull it from stock when the parent is built, whether you originally made it or bought it.

You don't tag this on each BoM line by hand. The method type lives on the item itself, and every BoM that uses that item mirrors it. Switch a part's method type once, and it cascades everywhere the part appears.
