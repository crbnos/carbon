...
Posting a receipt always updates inventory. The accounting entries — debit inventory or <Term id="wip">WIP</Term>, credit the <Term id="gr-ir">goods-received-not-invoiced (GR/IR)</Term> clearing account — are written only when accounting is enabled for the company. Inventory and the general ledger advance on different switches.
</Callout>
...
Posting the purchase invoice writes the payable side of the ledger — credit Accounts Payable, debit inventory or WIP, reverse the GR/IR clearing the receipt set up, and send any price difference to variance — all gated on accounting being enabled. It bumps each order line's `quantityInvoiced`, flips `invoicedComplete`, and recomputes the order's status. The invoice itself lands on **"Open"**.
