...
Eventually an asset leaves the books. There are two ways out, and Carbon treats them as genuinely different actions — handled by different teams, with different effects. You **sell** an asset to a buyer, or you **dispose** of it when it's done. Both start from an asset that's **"Active"** or **"Fully Depreciated"**.

The number that anchors both is **net book value** — what the asset is still worth on the books: its acquisition cost minus the depreciation accumulated against it.

## Sell it

Selling a piece of equipment is a sales act, so it lives with the sales team. The **Sell** action drafts a real **sales order** with a single **"Fixed Asset"** line pointed at the asset, priced at its **net book value**. From there it's the ordinary quote-to-cash flow — ship it, invoice it, collect — with the order line carrying a link back to the asset it came from.
