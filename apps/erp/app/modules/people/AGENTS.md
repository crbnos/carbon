...
- `getShiftsWithTimes` — shifts with `startTime`/`endTime` + per-weekday flags for a location (crew capacity math); `getEmployeeShifts` — company-wide `employeeShift` map (each person's standing shift)
