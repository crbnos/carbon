...
/**
 * Supplier returns connected to a purchase order — header-linked union
 * line-linked, deduplicated. Rendered in the PO properties panel.
 */
export const usePurchaseOrderReturns = (purchaseOrderId: string) => {
  ...
  useEffect(() => {
    ...
    ]).then(([headerReturns, lineReturns]) => {
      if (headerReturns.error || lineReturns.error) return;
      const byId = new Map<
        string,
        { id: string; purchaseReturnOrderId: string; status: string }
      >();
