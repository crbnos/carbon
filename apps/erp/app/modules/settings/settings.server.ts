...
// Server-only (the GoTrue admin wrappers carry the service-role key), so these
// live here rather than in settings.service.ts, which is re-exported by the
// client barrel. Route code imports them directly from this file. Domain and
// provider LOOKUPS live in @carbon/auth/sso.server (shared with MES and jobs);
// only the company-scoped read and the admin mutations belong here.
