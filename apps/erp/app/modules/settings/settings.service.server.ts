...
// `overrides` lets the onboarding flow pass the just-inserted companyId,
// which is not yet on the request's auth context.
export async function seedCompany(
  parentCompanyId?: string,
  overrides?: { companyId?: string; userId?: string }
) {
  const client = getAuthClient<SupabaseClient<Database>>();
  const ctx = AuthContextHolder.tryGet();
  const companyId = overrides?.companyId ?? ctx?.companyId;
  const userId = overrides?.userId ?? ctx?.userId;
  if (!companyId || !userId) {
    throw new Error(
      "seedCompany: companyId and userId required (no auth context and no overrides)"
    );
  }
...
export async function updateAssetTaxDepreciationSettings(settings: {
