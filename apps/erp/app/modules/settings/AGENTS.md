...
- `hasRevision` (an **uncommitted local addition** at time of writing) gates selecting `revisionId`: only the revised document views carry it, so requesting it unconditionally would error on the others. When set, the label is built with `withRevisionSuffix` (`@carbon/documents/utils`) so two revisions sharing one readable id don't render as indistinguishable options. Verify the current flag set on disk before relying on it.
