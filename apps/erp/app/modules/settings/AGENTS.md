...
- `updateLeasePolicySettings` (the ASC 842 lessor thresholds `leaseMajorPartThresholdPercent` / `leaseSubstantiallyAllThresholdPercent` and `leaseDefaultDiscountRate`, percentage points) / `updateRevenueRecognitionSetting` (`companySettings.revenueRecognitionEnabled`) — both written from `x+/settings+/accounting.tsx` (intents `leasePolicy` and `revenueRecognitionEnabled`); lease policy applies to later rental activations only
