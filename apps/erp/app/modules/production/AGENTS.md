...
- **Operation Batch** (`jobOperationBatch`) — N job operations sharing one run on a **batchable process** (`process.batchable`: laser table, furnace, plating). Jobs are never merged and the BOM is never modified; each member stays a first-class `jobOperation`. Time/cost split **proportional to `operationQuantity`**, materialized at completion as per-member `productionEvent` slices, so job costing/GL post per job with no special-casing. Status `Active → Completing → Completed` (no `Cancelled` — dissolve deletes a pre-start batch instead): completion is **two-phase and resumable** — Phase 1 (one txn) slices events + records quantities + flips `Active → Completing`; Phase 2 (idempotent, edge-fn-owned) issues each member's BOM, flips members `Done`, posts GL, then `Completing → Completed`. A Phase-2 failure leaves the batch `Completing` and re-invoking resumes (no double issue/GL). A retry MUST resubmit the quantities Phase 1 recorded — a changed quantity is rejected, not silently applied. Proportional split logic lives in `@carbon/utils` `batch-time-split` (Deno mirror under `supabase/functions/shared/`). Distinct from lot/batch tracking (`trackedEntity`/`batchNumber`). Mutations go through the `batch-operations` edge function (create/add/remove/update/dissolve/complete). Batch COMPOSITION lives on the operations schedule board (`x/schedule/operations`): selectable checkboxes on batchable unstarted ops + a floating create bar (`BatchSelectionProvider` — the bar also shows the setup-time saving `sum → max` and warns when member due dates spread ≥7 days; a floating banner surfaces same-process opportunities spanning 2+ work-center columns), and each live batch collapses to one `BatchItemCard` in its work-center column (drag across columns reassigns the batch work center; Completing cards are read-only). There is NO separate batching board/route — it was removed 2026-08-21. Completion (MES modal → `complete`) can mark a member "not in this run" (`excluded`): it detaches back to the schedule un-run inside the Phase-1 txn instead of flipping Done; excluded members carry no quantities, all-excluded is rejected (dissolve instead), and a resume drops already-applied exclusions but rejects new ones. Batches LIST at `/x/production/batches` (nav: Production → Batches; `getJobOperationBatches` + `getJobOperationBatchMemberStats` + drawer via `getJobOperationBatchWithMembers`), and a printable **load sheet** serves at `/file/batch/:id.pdf` (`BatchLoadListPDF` in `@carbon/documents`; menu entries on the batch card and MES batch chip). Job surfaces show live-batch membership: a Batches column on the jobs table (loader-side lookup), a badge on the `JobBillOfProcess` operation row (`jobOperationBatch` embed), and a chip on the MES job DAG node.
...
- `getJobOperationBatches` / `getJobOperationBatchMemberStats` / `getJobOperationBatchWithMembers` — the `/x/production/batches` list (filterable, member count + summed quantity merged in the loader) and its detail drawer
