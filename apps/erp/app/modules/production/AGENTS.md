...
| `crewAssignment` / `crewAbsence` | Manning board: person→work-center per date (optional shift; unique per person/day/shift) and person-out-for-date flags; consumed by the Crew page (`x+/schedule+/crew.tsx` — Board/Matrix/Capacity views via `?view=`; the route delegates to `ui/Schedule/Crew/` components: `CrewHeader`, `CrewBoard` + `CrewCard` + `CrewColumn`, `CrewWeekBoard`, `CrewMatrix`, `CrewCapacity`, `CrewHoursModal`, `OvertimeDialog`, `TimeOffDialog`, with the shift-hours/time ladders and shared types in `crewShared.ts`; the Capacity view's demand/scheduled buckets come from `buildCrewCapacityBuckets` in `crewCapacity.server.ts`), the MES station default, and the scheduling engine |
...
- `getCrewAssignments` / `getCrewAbsences` (single date) and `getCrewAssignmentsRange` / `getCrewAbsencesRange` (week) — crew board/matrix reads; `upsertCrewAssignment` (move semantics, optional partial-day `hours`) / `moveCrewAssignment` (station move with same-station merge, Kysely txn) / `setCrewAssignmentHours` (per-station hours; null = whole shift — lowering releases the remainder to the board's free-hours pool) / `setCrewAssignmentOvertime` / `setCrewOvertimeBulk` (one date or a `date`→`toDate` range — the board's Week horizon applies a whole week; optional department/shift scope, single UPDATE) / `deleteCrewAssignment` / `setCrewAbsence` / `setCrewAbsenceRange` (vacation as one action — a row per date, duplicates skipped) / `clearCrewAbsence` / `copyCrewBoard` (day; copies splits, not overtime) / `copyCrewWeek` (whole Monday-start week, same skip rules) / `assignCrewWeek` / `unassignCrewWeek` / `moveCrewWeek` (week-basis manning for the Board's Week range: one row per working day from the shift's weekday flags, Mon–Fri fallback) / `setCrewDay` (atomic reconciliation of a person's whole day — the `CrewHoursModal` Working-hours editor's Save, also anchored on Matrix cells) — crew mutations
