...
| `jobOperationBatch` | Lightweight join over N `jobOperation` rows sharing one **batchable** process (`process.batchable`) that a machine can run simultaneously (laser/heat-treat/plating). `readableId` `BAT` sequence, `status` `Active`/`Completed`/`Cancelled`, optional `workCenterId` (propagated to every member op), per-`locationId` planning board. Members carry `jobOperation.jobOperationBatchId`; running/sliced timers carry `productionEvent.jobOperationBatchId`. All lifecycle writes go through the **`batch-operations` edge function** (never direct) — it owns the eligibility gate and transactional writes. At completion the ONE recorded batch timer per type is sliced into per-member `productionEvent` rows proportional to each member's planned `operationQuantity` (`buildBatchCompletionPlan`, `@carbon/utils` `batch-time-split`, mirrored Deno-side in `supabase/functions/shared/batch-time-split.ts`), then each member issues its own BOM + flips Done + posts GL — so costing/estimates-vs-actuals show each job its proportional share with no special-case path |
...
- `getActiveJobOperationsByLocation` — schedule board data (RPC `get_active_job_operations_by_location`); also surfaces each operation's `jobOperationBatchId` so the schedule board / MES kanban can badge batched operations
...
- `getBatchableOperations` — the batch planning board's candidate pool (RPC `get_batchable_operations`): unstarted (`Todo`/`Ready`/`Waiting`), unbatched job operations whose **process** is `batchable`, for jobs at a location (`Ready`/`In Progress`/`Paused`). Each row carries a `materials` JSONB array of the resolved material facets (`substance`/`grade`/`dimension`/`finish`/`form`) for the operation's **own** BOM lines (`jobMaterial.jobOperationId` → `item` → `material`), so the board can facet-filter and group; operations with no material-bearing BOM lines return `[]` (grouped under "No material properties"). Cast rows via `BatchOperationMaterial`
- `getJobOperationBatches` — Active batches for a location's planning board
- `batchOperations` — thin wrapper over the `batch-operations` edge function; discriminated `type`: `create` / `add` / `remove` / `updateWorkCenter` / `dissolve` (membership) and `complete` (per-member event slicing + quantities + issue + Done + GL). The edge function rejects with a specific error on: mixing processes, a non-batchable process, a started operation, an already-batched operation, or any add/remove/dissolve after a batch production event exists (dissolve names the recovery: complete the batch)
