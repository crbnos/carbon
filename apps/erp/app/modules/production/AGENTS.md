...
- `syncAssemblyInstructionToOperation` — Assembly → BOP sync (Kysely txn): copies a Published instruction's steps into a method/job operation as `methodOperationStep`/`jobOperationStep` rows (typed fields mirror 1:1), links per-step BOM parts via the material↔step join tables (itemId resolved against the target operation's own materials), attaches the instruction's 3D model as a model slide on every synced step, and sets the operation's `assemblyInstructionId`. Re-runnable via the `assemblyInstructionStepId` provenance marker (update matched / insert new / delete stale; hand-authored NULL-marker steps untouched; ON DELETE SET NULL demotes to hand-authored). Route `x+/assembly+/$id.sync-bop.tsx` (loader = targets, action = guarded sync: Draft method / unlocked job, `parts_update` vs `production_update`); UI = "Sync to BOP" on a Published instruction's header (`AssemblySyncModal`). get-method's job-step copy spreads all columns, so markers flow method → job (and get-method copies `assemblyInstructionId` onto job operations). The MES assembly view uses the marker for **step-aware 3D playback**: `getAssemblyPlaybackByOperationId` (apps/mes operations.service) loads the linked instruction's GLB/graph/steps and `AssemblyView` mounts the `@carbon/viewer` `AssemblyPlayer` pinned to the operator's current step
