...
function CandidateCard({
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`cursor-grab rounded-lg border bg-card p-3 shadow-button-base ${
...
function BatchLane({
  ...
  const { t } = useLingui();
  const { setNodeRef, isOver } = useDroppable({ id: `batch:${batch.id}` });
  ...
  return (
    <div
      ref={setNodeRef}
      className={`rounded-xl border p-2 ${isOver ? "border-primary bg-muted/40" : ""}`}
    >
