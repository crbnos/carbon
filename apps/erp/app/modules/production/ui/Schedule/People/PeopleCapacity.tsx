...
export function PeopleCapacity({
  ...
  const availableHours = useCallback(
    (workCenterId: string, date: string) => {
      const people = peopleHours.get(`${workCenterId}:${date}`) ?? 0;
      if (people > 0) return people;
      // unassigned fallback: the location's shift calendar for that weekday
      return calendarHoursByDate[date] ?? 0;
    },
    [peopleHours, calendarHoursByDate]
  );
          ...
          <TooltipContent align="start" className="max-w-[320px]">
            <div className="flex flex-col gap-1.5 text-xs">
              ...
              <p>
                ...
                —{" "}
                <Trans>
                  the shift hours of the people you've assigned. Stations with
