...
import { chargeStatus, chargeType } from "../../invoicing.models";
...
const ChargesTable = memo(({ data, count }: ChargesTableProps) => {
  const { t } = useLingui();
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <Hyperlink to={path.to.charge(row.original.id)}>
      ...
      },
      {
        ...
        meta: {
          ...
          filter: {
            type: "static",
            options: chargeStatus.map((status) => ({
              value: status,
              label: <ChargeStatus status={status} />
            }))
          },
  ...
  return (
    <Table<ChargeRow>
      ...
      columns={columns}
      data={data}
      defaultColumnPinning={{ left: ["chargeId"] }}
