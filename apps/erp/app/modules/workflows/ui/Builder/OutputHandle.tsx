...
// Long enough that a list never grows past the card it hangs off.
const MAX_ROWS = 12;

function OutputList({ variables, routesOnly }: HandlePreview) {
      ...
      {groups.map((group) => (
        <div key={group.nodeName} className="px-2 py-1.5">
          ...
          {group.rows.map((row) => (
            <div
              key={row.output}
              className="truncate pt-0.5 font-mono text-xs text-foreground"
            >
              {row.output}
            </div>
          ))}
  ...
  return (
    <Popover open={preview !== null}>
      ...
      <PopoverContent
        ...
        className="pointer-events-none w-64 p-0"
