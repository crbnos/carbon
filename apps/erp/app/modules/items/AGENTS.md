...
- **Stage flow** — `changeOrderStatus`: `Draft` → `Start` → `Engineering Complete` → `Implementation` → `Done`, plus a `Cancelled` off-ramp. Table-driven, one step forward (`changeOrderStatusTransitions` / `isAllowedChangeOrderTransition` — the check is purely `allowed.includes(to)`, NOT directional), with two backward **reopen** edges: `Implementation → Engineering Complete` and `Cancelled → Draft`. Both are ⋮-menu items in `ChangeOrderHeader`. `Done` is terminal — releasing already activated methods and wrote supersessions, so it cannot be reopened. **The reopen edge must stay LAST in the `Implementation` array**: the header reads `transitions[status]?.[0]` for its advance/release action. `changeOrderOpenStatuses` = every stage before Done. `Start`/`Implementation`/`Done` broadcast a team notification (`changeOrderBroadcastStages`).
- **Two lock scopes** (`items.models.ts`) — `isChangeOrderLocked(status)` is true at `Done`/`Cancelled` (fully closed). On top of that: `canEditChangeOrderEngineering(status)` additionally excludes `Implementation` and gates affected items (add/remove/change-type), the staged BOM/BOP drafts, cutover config, and `reasonForChange`/`description`; `canEditChangeOrderWorkflow(status)` is the closed-only test and gates action tasks, assignee, dates and priority — those stay editable at `Implementation`, which is the whole point of that stage. (`canEditChangeOrder` was split into these two and no longer exists.) Server enforcement is `requireChangeOrderEditable(client, { changeOrderId, companyId, scope })` in `items.server.ts`; the UI `isDisabled` is cosmetic on top of it. BOM/BOP mutations are covered centrally — `checkRevisionLock` also resolves the owning CO's status, so all ~17 `methods+/` routes inherit the lock without their own guard. Note `item.changeOrderId` is a PERMANENT back-link (unlike `makeMethod.changeOrderId`, cleared at release), so it must never be used to resolve ownership.
...
- MUST gate every CO mutation route with `requireChangeOrderEditable(client, { changeOrderId, companyId, scope })` from `items.server.ts` — `scope: "engineering"` for affected items / BOM / BOP / cutover / reason / description, `scope: "workflow"` for action tasks and header fields. A UI-only `isDisabled` is not a lock.
