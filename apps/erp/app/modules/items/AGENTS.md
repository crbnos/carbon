...
- Assume `methodMaterial.sourcingType`/`methodType`/`replenishmentSystem`/`itemTrackingType` mirror the component item — they're **editable per BoM line** now, defaulting to the item but overridable. A per-line edit sets the matching `sourcingTypeOverridden`/`methodTypeOverridden`/`replenishmentSystemOverridden`/`itemTrackingTypeOverridden` flag; `upsertMethodMaterial` then keeps the submitted value instead of re-deriving, and the item-default cascade updates only **non-overridden** lines (`cascadeSourcingAndMethodTypeToMethodMaterials` for the first three; `cascadeItemTrackingType` for tracking type) — the interlocked trio (`sourcingType`/`replenishmentSystem`/`methodType`) cascades Draft-only (frozen methods keep valid pairs), while `itemTrackingType` cascades on ALL statuses (live mirror; the ledger pipeline depends on it). Effective columns stay `NOT NULL` — `get_method_tree`/MRP read them unchanged (no MRP change). **`itemTrackingType` is the one with real downstream effect:** `get-method` snapshots the per-line value onto `jobMaterial.itemTrackingType` and the `issue` edge function reads it, so a line flipped to `Non-Inventory` is consumed with no stock ledger entry. It's restricted to Inventory↔Non-Inventory (Serial/Batch stay item-level); receipt/shipment (not BoM-line driven) still read `item.itemTrackingType`. INSERT-time inheritance is DB-owned: the `methodMaterialInheritItemDefaults` BEFORE INSERT trigger fills unflagged `sourcingType`/`replenishmentSystem`/`itemTrackingType` from the component item, so bulk insert paths (get-method copy-backs, import-csv) can't leave stale defaults; `methodType` stays writer-owned (it encodes tree structure on copy-backs).
