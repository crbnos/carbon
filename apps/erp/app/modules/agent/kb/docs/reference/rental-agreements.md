...
## Lifecycle
  ...
  - **Draft**: Terms and units are editable. The units are already held for this agreement.
  - **Active**: Activated: rates are locked onto each unit and billing periods are cut. Units are delivered, returned, and billed.
  - **Closed**: Every unit returned or sold, every billing period and charge invoiced.
  - **Cancelled**: Abandoned before anything was billed; units are released.
...
With a best rate unit on a **28 Days** agreement, each period bills the cheapest of its tiers in whole units, and a tie goes to the larger unit. With rates of 100 a day, 500 a week, and 1,500 a month:
...
Billing periods follow the **"Billing Cycle"** from the start date, and the **"Billing Timing"** decides when each one falls due: *Advance* on its first day, *Arrears* on its last. The **Billing Periods** section of the agreement shows every period with its days, tier, amount, and invoice.
...
| Line | Posts to |
| --- | --- |
| Rent | Contract Assets for the part already accrued, then Deferred Revenue for the rest |
...
A refundable deposit is a customer deposit, not revenue. The agreement's **Deposits** section shows the **"Agreed deposit"** and the payments made against it; **"Record Deposit"** opens a prefilled customer receipt.

On any customer receipt, **"Deposit for"** names the sales order or rental agreement the money secures. The receipt's unapplied cash then posts to **Customer Prepayments** (2110) instead of on-account credit. Applying it to an invoice later moves it from Customer Prepayments to receivables, and a disbursement with **"Refund deposit for"** pays the balance back out of Customer Prepayments.
...
**"Override"** on a draft agreement's unit sets the classification by hand with a required **"Reason"**. It needs accounting update permission and is kept by activation in place of the tests' answer. The unit keeps the reason, and with the `docs/reference/audit-log` on, the change is also recorded there with the old and new values. Once the agreement is active the classification is fixed.
...
The monthly **Revenue Recognition** run posts each line's interest in its period: Dr Net Investment in Leases, Cr Interest Income – Leases. The invoices collect the balance:
...
## Related
  ...
  - Fixed assets The fleet register, and how a unit becomes an asset in the first place.
