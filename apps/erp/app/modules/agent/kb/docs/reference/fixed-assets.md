...
- **Capitalize** a unit already in stock: on the item's inventory page, the storage-unit row action **"Capitalize as Fixed Asset"** moves one serialized unit out of stock at its carrying cost and creates the asset, or fills a Draft one, as **Active**. The serial number travels with it.
...
A unit can also go back to stock. **"Return to Inventory"** on an Active or Fully Depreciated asset with an item puts the same serial back on the shelf at net book value, and marks the asset Disposed.
