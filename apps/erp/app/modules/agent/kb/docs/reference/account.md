...
visible to all users, so be careful what you share."* (`.../ui/Profile/ProfileForm.tsx:44`).
  ...
  - **First name / Last name**: Your display name across Carbon. Both are required.
  - **Phone**: Optional. A phone-formatted field, blank by default.
  - **About**: A short bio, capped at 160 characters (`ProfileForm.tsx:62`).
  - **Photo**: Your avatar. Uploading routes the image through Carbon's `image-resizer` before it's saved (`.../ui/Profile/ProfilePhotoForm.tsx`); you can also remove it and fall back to your initials.
  - **Email**: Shown, but **read-only** — the field is disabled (`ProfileForm.tsx:53`). You can't change your sign-in email from here.
...
`docs/reference/notifications`. This page is where you tune it for yourself.
...
*you* can edit your own value depends on it being marked self-managed.
...
**Settings**, and read the `docs/reference/notifications` for how alerts are
