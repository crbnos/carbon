...
> The two company-scoped lookups that sit alongside change notices — the category list and the reusable action-task templates — plus linking an action task to Linear or Jira.
...
it. This works exactly the way it does on a `docs/reference/quality` action tasks — same
dialog, same behavior.

The affordance is an icon on the task row, and it only appears when your company has that
`docs/integrations/project-management` connected: a Linear icon when Linear is enabled, a Jira
...
Opening the dialog gives you two tabs:
  ...
  - **Link Existing**: Search your Linear or Jira issues and attach one to this task.
