...
The Hub is organized around six sequential **gates** — each phase ends at exactly one milestone you sign off before moving on (`packages/onboarding/src/content/spine.ts:11-143`):
  ...
  - **Discovery**: Confirm how your company runs and lock the project scope. Ends at **"Scope signed"**. Paid tiers only.
  - **Configure**: Set Carbon up around your business: sites, users, suppliers, parts, BOMs, and Bill of Process. Ends at **"System configured"** — this is the phase most of your setup work lives in.
  - **Migrate data**: Load your real data and approve a validated sample. Ends at **"Data validated"**. Paid tiers only.
  - **Train**: Your team leads learn Carbon via the in-app guides and Academy, then bring everyone else up. Ends at **"Team trained"**.
  - **Acceptance**: Every in-scope test passes on your configured system, with your data. Ends at **"Acceptance passed"**. Paid tiers only.
  - **Go-Live**: Cut over, freeze the old system, and confirm you're live on Carbon. Ends at **"Live on Carbon"**.
...
The single most useful page for a hands-on operator is the **Setup Map** (in the "Configure Carbon" group). It's the first-run configuration checklist: every master, reference, and config entity a new company sets up before it starts transacting, grouped by module and labelled as it appears in that module's navigation (`packages/onboarding/src/content/setup.ts:32-436`). It covers, per module:
  ...
  - **Accounting**: Fiscal year, accounting periods, chart of accounts, default accounts, cost centers, dimensions, payment terms, exchange rates, asset classes.
  - **Settings**: Company identity, document templates, logos, printing, sequences, custom fields, integrations, approval rules.
  - **Resources**: Locations, work centers, processes, training abilities, failure modes, maintenance schedules.
  - **People**: Employees, departments, shifts, holidays, attributes, employee types, permission groups.
  - **Items**: Units, item groups, parts, materials, tools, consumables, and the material substances, shapes, grades, and finishes.
  - **Sales**: Customers, customer types and statuses, price lists, pricing rules, no-quote reasons.
  - **Purchasing**: Suppliers and supplier types.
  - **Inventory**: Storage units, storage types, storage rules, shipping methods.
...
The wizard gets you a tenant; the Hub gets you to production. For the settings the wizard seeds and you refine, see `docs/reference/company-settings`. For adding the rest of your team, their access, and permission groups, see `docs/reference/people`. And for the end-to-end story of how work actually flows through Carbon once you're set up, the [guides](/guides) walk it start to finish.
