...
The memo amount is quantity × unit price, less the restock fee. Creating it does not post the memo or settle cash. Open it under Vendor Credits and choose **Post**. A Draft memo already reserves the creditable quantity, while the Supplier Returns list counts only Posted credit in its **Credited** total.
