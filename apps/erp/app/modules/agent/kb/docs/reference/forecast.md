...
Two tables look alike. `demandProjection` is your input — the numbers you type. `demandForecast` is planning's output — the exploded, per-component demand the MRP run writes back (`forecastMethod` = `"mrp"`). You never edit `demandForecast` by hand; it's rebuilt on every run (`packages/ee/src/planning/mrp/run-mrp.ts:918-963`). The Projections screen only ever touches `demandProjection`.
