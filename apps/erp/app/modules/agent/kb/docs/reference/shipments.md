...
A **shipment** moves goods out the door. One shipment model serves several source documents: a sales order, a purchase order, an outbound transfer. Each is tagged by what it came from, so the same posting logic handles them all.
...
The full message names the batch id. The company's expired-stock rule is **Block** (or **Block with override**) and a batch on the shipment is past its expiration date. Options: allocate a different batch, correct the expiration date on the tracked entity (reason required), or change the rule in Settings → Inventory to Warn. See `docs/reference/shelf-life`.
