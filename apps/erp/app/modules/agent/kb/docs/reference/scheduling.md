...
Scheduling in Carbon answers two questions for the floor: *when* does each `docs/reference/jobs` run, and *in what order* does each `docs/reference/work-centers` pick up its operations. The **schedule board** in the ERP is where you set the first, and an engine derives the second. Both live on one board with two views, reached from the **Production** area.
...
## Related
  ...
  - Jobs The work orders whose operations the schedule board sequences.
  - Work centers The stations operations are placed on, and the rates that cost them.
  - Plan the build How demand turns into scheduled jobs before they hit the floor.
