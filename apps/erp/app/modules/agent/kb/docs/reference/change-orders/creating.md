...
the change order's first affected item, so you land ready to pick a change type and edit. The standalone form
...
part, tool, or quality issue, that source is carried silently onto the new change order (the part or tool as a
pre-attached affected item, the non-conformance as the linked issue). Submitting creates the change order,
attaches any pre-selected item, and redirects to its detail page at **"Draft"**.

Change orders are gated by the **Parts** permission: creating one needs create access on Parts, and
advancing or releasing needs update access. If someone can manage parts, they can manage change orders.
...
- **Provenance back-link.** A revision or part created by a released change order carries a *"Created by
  ECO-…"* reference to the change order that made it.

Each change order gets a per-company readable id like **ECO-000001**. That's the id you'll see in the alert,
