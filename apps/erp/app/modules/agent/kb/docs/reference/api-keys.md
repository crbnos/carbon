...
Carbon shows the full key exactly once, in a dialog headed "You can only see this key once. Store it safely." Copy it right then. Carbon never stores the raw key: it keeps only a SHA-256 hash for lookup and the five-character preview for display. There is no way to reveal a key again later — if you lose it, **regenerate** the key (below) rather than deleting and recreating it.

Anyone holding the key can act as it, within its scopes. Treat it like a password: store it in a secret manager, never commit it to source control, and regenerate it if it leaks.
...
**Settings → API Keys → the key's context menu → Regenerate API Key** issues a new secret for an existing key. The row keeps its id, name, scopes and expiration; only the secret changes, so every integration you point at the new value keeps exactly the access it had.

The old secret stops working the instant you confirm — there is no grace period and no overlap window. Anything still sending it gets "API key not found" on its next request, so have somewhere to paste the new value before you start. The confirmation dialog reports when the key last authenticated a request, which is the quickest way to tell whether something live is about to break.

Regeneration is refused on an expired key — a fresh secret on an expired row would be rejected on its first request — so extend or clear **Expires At** first. It also clears the key's last-used timestamp, since that recorded the retired secret, and stamps who regenerated it and when. The new key is shown once, in the same dialog creation uses, with the same ready-to-paste MCP command.

Use this instead of delete-and-recreate for a lost or leaked key: recreating means re-entering the name and re-granting every scope, and any integration pointed at the old key id has to be rewired.
...
There's no way to reveal a key after creation. Carbon shows the full key exactly once, in the "You can only see this key once. Store it safely." dialog, and keeps only a hash afterward. If you lost it, use **Regenerate API Key** from the key's context menu: it issues a new secret on the same row, keeping the name, scopes and expiration. The old secret dies immediately, so update every caller. Deleting and recreating also works but throws away the scope configuration.
