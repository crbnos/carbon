...
Cash and memos both settle open items through one primitive: the **invoice settlement**. Each row nets one **source** against one **target**.
  ...
  - **sourcePaymentId**: A *prior* posted payment supplying on-account credit. `NULL` means the row is funded by the applying payment's own cash. Either way `paymentId` stays the applying and voiding owner, so a credit-funded row can only sit on a cash payment, never on a memo.
  ...
  - **sourceAmount**: Principal consumed **in the funding source's document currency**, stored independently of `appliedAmount`.
  - **appliedAmount**: Target-document principal relieved, **in company base currency**.
  - **discountAmount**: An early-payment discount you're granting, in **base currency**. **Cash payments only** — a memo-sourced settlement can't carry one.
  - **writeOffAmount**: A remainder you're forgiving to the write-off account, in **base currency**. **Cash payments only.**
  - **fxGainLossAmount**: The realized FX this one application books, in **base currency**: positive is a gain, negative a loss. Server-calculated at posting, never entered.
  - **appliedDate**: When the application takes effect.

A settlement row has to move something: either applied, discount, and write-off sum to more than zero, or the row consumes a positive `sourceAmount`. An AR source only settles AR targets for the same counterparty, and an AP source only settles AP targets — the posting functions enforce party and side consistency (a Receipt rejects purchase invoices, a Disbursement rejects sales invoices, and a payment can only settle its own party's invoices).
...
- **A payment** books the **bank account** against the **AR or AP control account** (receivables for a customer, payables for a supplier), split per application. Each settlement's discount goes to the discount account and its write-off to the write-off account. Realized FX is computed **per application** — the carrying value released from the funding source, less the base principal relieved on the target — and stored on that settlement row as `fxGainLossAmount`. The journal then books the **net** of those to the realized exchange gain or loss account as one line.
- **A memo** books a balanced two-line entry: the **control account** against the derived **reason account**. Applying a balance-reducing memo to an invoice afterward is GL-neutral — both sit in AR/AP — and realizes no FX, because Carbon only lets you apply a credit whose exchange rate matches the invoice's.
