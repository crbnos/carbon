...
A **change order** groups the parts whose designs are changing, revises each one on a hidden draft, and
...
A change order is a header plus a list of **affected items**. The picker accepts **Parts only** — materials,
consumables, services, and tools have no path in the change-order UI, so they're excluded. Each affected item
...
shows a single id — it's net-new, with no predecessor to arrow from. You see what a change order touches
without opening it.
...
## How the pieces fit
  ...
  - Create a change order Every entry point — the standalone form, a part header, the parts table, the method version menu, and a
    quality issue — plus how change orders surface back on a part.
