...
A **change notice** groups the parts and tools whose designs are changing, revises each one on a hidden
...
A change notice is a header plus a list of **affected items**. The picker accepts **Parts and Tools only** —
...
what a change notice touches without opening it.
...
## How the pieces fit
  ...
  - Categories & actions The change-notice category lookup, the reusable action-task templates, and linking a task to Linear or
    Jira.
