...
flowchart TB
  ...
  subgraph apps["Our Node.js apps"]
    ERP["ERP app - the main product"]
    MES["MES app - shop floor"]
  end

  subgraph sb["Supabase services"]
    REST["PostgREST - turns HTTP calls into SQL"]
    AUTH["Auth - logins, tokens, invites"]
    STORE["Storage - files, PDFs, CAD models"]
    RT["Realtime - streams row changes to browsers"]
    EDGE["Edge Functions - small Deno programs"]
    PG[("Postgres - the source of truth")]
  end

  JOBS["Inngest - background job runner"]
  REDIS[("Redis - cache, locks, rate limits")]
  ASM["Assembler - Rust service for 3D and CAD"]
  EXT["Outside world - email, Slack, Stripe, Jira, Xero"]
...
flowchart LR
  CODE["Server code - loader, action, or job"]

  A["supabase-js - user scoped"]
  B["supabase-js - service role"]
  C["Kysely - direct SQL"]
  D["Edge function - Deno"]
...
flowchart LR
  subgraph write["Write path"]
    A1["action"] --> A2["service upsert"] --> A3[("purchaseOrder table")]
  end
  subgraph read["Read path"]
    B1["loader"] --> B2["service get"] --> B3["purchaseOrders view"]
  end
...
flowchart TB
  W["Any write to purchaseOrder<br/>from anywhere at all"]
  T["Postgres trigger - dispatch_event_batch<br/>runs inside the same transaction"]
  Q[("PGMQ - a queue stored in Postgres tables")]
  BELL["util.wake_event_queue - fires an HTTP ping after commit"]
  EF["event-wake edge function"]
  ING["Inngest"]
  DR["event-queue job - drains the queue, groups by type"]

  H1["SEARCH - update the global search index"]
  H2["WORKFLOW - run customer automations"]
  H3["WEBHOOK - POST to a customer's URL"]
  H4["AUDIT - write the audit log"]
  H5["SYNC - push to Xero, Jira, Linear"]
  H6["EMBEDDING - vectors for AI search"]
...
flowchart TB
  subgraph dev["Your laptop"]
    CLI["pnpm dev - runs the crbn CLI"]
    DOCK["Docker: Postgres, PostgREST, Auth, Storage,<br/>Realtime, Kong, edge runtime, Studio,<br/>Inbucket mail catcher, Inngest"]
    VITE["Vite dev servers for ERP and MES"]
  ...
  subgraph ci["GitHub Actions on merge to main"]
    CHK["check.yml - lint, typecheck, tests"]
    DEP["deploy.yml - build Docker images, push to ECR"]
    MIG["supabase.yml - apply migrations and edge functions"]
    JOB["inngest.yml - register job functions"]
  end

  subgraph prod["AWS - one stack per customer, defined in sst.config.ts"]
    ALB["Load balancer plus WAF rate limiting"]
    ECS["ECS Fargate: ERP containers and MES containers<br/>auto-scaling 1 to 10"]
    SUPA[("Managed Supabase - Postgres")]
    ALB --> ECS --> SUPA
  end
