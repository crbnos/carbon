...
> An RFQ becomes a quote; an accepted quote becomes a sales order.
...
There is no path from a quote straight to a production job. A `guides/order` line on the *sales order* is what spawns a job, so the sequence is always quote → order → job. The order is the source of truth the floor builds against.
