...
export async function uploadStepSlideImage(
  ...
  // Best-effort thumbnail creation for step slide
  try {
    const thumbFile = await createResizedImageThumbnail(file, SUPABASE_URL);
    if (thumbFile) {
    ...
    // Non-fatal: original remains available
  }
                  ...
                  {slide.modelUploadId ? (
                    ...
                    <>
                      <img
                        src={
                          slide.imagePath
                            ? getPrivateUrl(getThumbnailPath(slide.imagePath))
                            : ""
                        }
                        onError={(e) => {
                          if (
                            slide.imagePath &&
                            e.currentTarget.src !==
                              getPrivateUrl(slide.imagePath)
                          ) {
                            e.currentTarget.src = getPrivateUrl(
                              slide.imagePath
                            );
                          }
                        }}
