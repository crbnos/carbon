...
import { getSupplierQuoteByExternalLinkId } from "~/modules/purchasing/purchasing.service.server";
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  // Public digital-quote response route: no session/middleware identity.
  ...
  // so services reading `AuthContextHolder.get()` are satisfied.
    ...
    async () => {
      ...
      switch (intent) {
        ...
        case "submit": {
          ...
          const parseResult =
            nestedSelectedLinesValidator.safeParse(parsedData);
          ...
          for (const [lineId, lineSelections] of Object.entries(
            selectedLines
          )) {
          ...
          if (companySettings.data?.supplierQuoteNotificationGroup?.length) {
            try {
              ...
              console.error(
                "Failed to trigger supplier quote notification",
                err
              );
            }
          }
