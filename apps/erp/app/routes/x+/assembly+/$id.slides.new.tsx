...
import { upsertAssemblyInstructionStepSlide } from "~/modules/production";
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const upsert = await upsertAssemblyInstructionStepSlide(
    client,
    id
      ? {
          id,
  ...
  if (upsert.error) {
    return data(
      ...
      await flash(
        request,
        error(upsert.error, "Failed to save assembly step slide")
      )
    );
  }
