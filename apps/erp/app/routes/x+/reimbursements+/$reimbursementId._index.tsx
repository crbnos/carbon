...
import { getReimbursement, ReimbursementSummary } from "~/modules/invoicing";
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const [accounts, journal, mapping, dimensions] = await Promise.all([
    accountIds.length > 0
      ? client
          .from("account")
    ...
    // (or there is no mapping), fall back to null silently so the SOURCE badge
...
export default function ReimbursementDetailRoute() {
  const { reimbursement, lines, accountsById, journal, mapping, dimensions } =
