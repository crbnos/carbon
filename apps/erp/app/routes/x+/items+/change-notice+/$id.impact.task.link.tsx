...
import { linkAuthorizedChangeNoticeImpactTask } from "~/modules/items/items.server";
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const result = await linkAuthorizedChangeNoticeImpactTask({
    client,
    companyId,
    userId,
    changeNoticeId,
  ...
  if (result.error) {
    return data(
      { success: false },
      await flash(request, error(result.error, "Failed to link Impact task"))
    );
  }
