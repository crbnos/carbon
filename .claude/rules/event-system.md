...
- `attach_event_trigger(table, sync_functions[], after_sync_functions[])` — helper that wires the BEFORE SYNC / AFTER SYNC / ASYNC STATEMENT triggers on a table (3rd arg added `20260410030406`). **Two overloads are live** — `(TEXT,TEXT[])` and `(TEXT,TEXT[],TEXT[])` — because `CREATE OR REPLACE` with an added parameter creates a sibling rather than replacing. Adding a *defaulted* parameter to this function is therefore a trap: a 2-arg call then matches more than one candidate and fails with `function attach_event_trigger(unknown, text[]) is not unique`, breaking 62 call sites. Adding an argument means dropping both overloads first.
