...
This is the Carbon-adapted variant. Carbon is a pnpm + turbo monorepo (`apps/erp`, `apps/mes`, `apps/academy`, `apps/starter`; `packages/*`) for a manufacturing ERP/MES system. It ships its own agent knowledge base under `.claude/rules/` (auto-loaded by code path) — recon **starts there**, not by reading source.

## Hard Rules

1. **Never modify source code yourself.** No edits, no fixes, no "quick wins while you're in there." The ONLY files you may create or modify live under `.claude/scratch/plans/improve/` (create it if absent). Never write to `.claude/rules/` — that's the committed-knowledge base, and plans describe uncommitted future work (per `AGENTS.md`: never cache plans or not-yet-committed code). The `execute` variant dispatches a *separate executor subagent* that edits code in an isolated git worktree — you review its diff and render a verdict; you still never edit code directly, and you never merge, push, or commit to the user's branch.
...
3. **Every plan must be fully self-contained.** The executor has not seen this conversation, this survey, `.claude/rules/`, or any other plan. If a plan references "the pattern discussed above" or "see the rules," it is broken — inline the relevant excerpt.
...
Map the territory before judging it. **In Carbon, recon means reading the `.claude/rules/` knowledge base first** — it is faster and higher-signal than reading source, and `AGENTS.md` mandates it.
...
- **Read the agent knowledge base** (these are Carbon's intent/design/ADR docs — treat decided tradeoffs here as by-design, not findings):
  ...
  - `.claude/rules/*.md` — domain knowledge per subsystem (auth, MRP, traceability, event-system, inventory, scheduling, printing, etc.). These auto-load for matching code and can be read directly — lean on the grounded rules instead of re-deriving from source. `mcp-tools-reference.md` is a normal, concise rule.
  - `.claude/rules/conventions-*.md` — forms, services, ui, database conventions. Plans must tell the executor to **match** these.
  - `.claude/scratch/tasks/lessons.md` — hard-won corrections (RLS pattern, upsert audit-field clobbering, `.merge()` after `.refine()`, `accountId` not `accountNumber`, enum conventions, etc.). **A finding that contradicts a lesson is by-design — do not report it.**
  - `.claude/rules/workflow-*.md` — `workflow-database-migration.md`, `workflow-edge-function.md`, `workflow-event-system.md`. Any plan touching these areas must cite and follow the workflow.
  - `.claude/scratch/recommendations/*.md` — existing forward-looking proposals (don't duplicate them in Direction).
...
- which `.claude/rules/` and `.claude/rules/conventions-*` files are relevant so the subagent reads Carbon's real conventions, plus the instruction that anything matching `.claude/scratch/tasks/lessons.md` is settled,
...
**Vet before presenting — subagents over-report.** For every finding that will make the table, open the cited code yourself and confirm it. Expect three failure classes: **by-design behavior** reported as a bug (e.g. a pattern that's actually the documented convention in `.claude/rules/conventions-*` or a settled call in `.claude/scratch/tasks/lessons.md`); **mis-attributed evidence** (real finding, wrong file or line); and duplicates across subagents. Downgrade, correct, or reject accordingly, and record rejections in the index's "considered and rejected" section so they aren't re-audited next run.
...
Present **direction findings separately**, after the table — 2–4 grounded suggestions max, each with evidence and trade-offs in two or three sentences. Cross-check `.claude/scratch/recommendations/` first so you don't re-propose what's already on file.

Then ask which findings to turn into plans (default suggestion: the top 3–5 plus anything they flag). Surface **dependency ordering** (e.g. "characterization tests for `@carbon/<pkg>` must land before the refactor"). Wait for the selection. Do not write 30 plans nobody asked for. If running non-interactively, write plans for the top 3–5 by leverage and record that default in `.claude/scratch/plans/improve/README.md`.
...
Before writing anything: record `git rev-parse --short HEAD` — every plan stamps the commit it was written against (the executor uses it for drift detection). If `.claude/scratch/plans/improve/` already exists from a previous run, **reconcile, don't duplicate**: read its README, keep numbering monotonic, skip findings already planned or listed as rejected, and mark superseded plans stale.

Write each plan **for the weakest plausible executor**:

- All context inlined: why it matters, exact file paths, current-state code excerpts, the Carbon conventions to follow (with a snippet of an existing exemplar file and the relevant `.claude/rules/conventions-*` rule quoted inline).
...
- For anything touching DB schema: the plan must point at `.claude/rules/workflow-database-migration.md` and state that **the user runs the migration**, not the executor.

Finish by writing `.claude/scratch/plans/improve/README.md` with the recommended execution order, dependencies, and a status column.
...
- `next` (or `features`, `roadmap`) → run Recon, then audit only the direction category in more depth: 4–6 grounded suggestions, each with evidence, trade-offs, coarse effort. Selected ones become design/spike plans. Check `.claude/scratch/recommendations/` first.
- `plan <description>` → skip the audit; the user already knows what they want. Run Recon (read the relevant `.claude/rules/` + `.claude/rules/conventions-*`), investigate just enough to specify it properly, write a single plan. Resolve ambiguity from the codebase first; only what's left becomes questions, asked one at a time with a recommended answer.
- `review-plan <file>` → critique an existing plan in `.claude/scratch/plans/improve/` against the template's standards and tighten it. If you authored it this session, also have a fresh-context subagent read it cold and report ambiguities.
